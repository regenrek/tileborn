...
const installCleanRuntimeFixture = async (root: string): Promise<string> => {
  ...
  await mkdir(path.join(pluginRoot, 'dist'), { recursive: true });
  const runtimeEntry = path.resolve(import.meta.dirname, '__fixtures__/playtest-runtime-clean.mjs');
  await writeFile(
    path.join(pluginRoot, 'tileborne-plugin.json'),
    JSON.stringify(
      {
...
describe('MapEditorPage mounted playtest ownership integration', () => {
  ...
  it(
    'drives owner cleanup through the mounted page, viewport, control hook, IPC handler, service, and runtime',
    async () =>
  ...
  it(
    'does not stop a mounted playtest during StrictMode effect replay',
    async () =>
      withTempHome(async (home) => {
        const harness = await createHarness();
  ...
  it(
    'deduplicates a successful registered drawer stop when viewport cleanup unmounts before it settles',
    async () =>
      withTempHome(async (home) => {
        const harness = await createHarness();
        const pluginRoot = await installCleanRuntimeFixture(home);
              ...
              stop: (input: unknown) => {
                ...
                if (
                  ...
                  return registeredStop.then(
                    ...
                    (error: unknown) => drawerStopGate.promise.then(() => Promise.reject(error)),
                  );
                }
  ...
  it(
    'retries the registered stop handler when unmount happens before explicit stop rejection settles',
    async () =>
      withTempHome(async (home) => {
        const harness = await createHarness();
