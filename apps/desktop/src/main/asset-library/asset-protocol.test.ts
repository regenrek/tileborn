...
describe('parseAssetProtocolRequest', () => {
  ...
  it('returns null for a thumbnail-host URL (handled separately)', () => {
    expect(
      parseAssetProtocolRequest(thumbUrl('pack:x', { assetPath: 'a.png', x: 0, y: 0, width: 32, height: 32 })),
    ).toBeNull();
  });
});
...
describe('parseAssetThumbnailRequest', () => {
  ...
  it('returns null for a pack-host URL, missing crop, or non-positive size', () => {
    ...
    expect(
      parseAssetThumbnailRequest(`${ASSET_PROTOCOL_SCHEME}://thumb?id=x&path=a.png&x=0&y=0&w=0&h=16`),
    ).toBeNull();
    expect(
...
describe('thumbnailCacheDir', () => {
  it('is content-addressed by pack id + integrity hash under the cache root', () => {
    ...
    expect(dir).toBe(path.join('/home/cache', 'asset-library/thumbnails', 'v1-pack_abc-sha256_def'));
  });
});

describe('computeThumbnailResize', () => {
  it('downscales the longest side into the box and preserves aspect', () => {
    const plan = computeThumbnailResize({ width: 1515, height: 1504 }, { x: 0, y: 0, width: 256, height: 128 }, 64);
  ...
  it('does not upscale crops already within the box', () => {
    const plan = computeThumbnailResize({ width: 256, height: 256 }, { x: 0, y: 0, width: 32, height: 32 }, 64);
  ...
  it('clamps a crop that overflows the source bounds', () => {
    const plan = computeThumbnailResize({ width: 40, height: 40 }, { x: 32, y: 32, width: 32, height: 32 }, 64);
