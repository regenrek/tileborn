...
export const invokePluginEditorCommand = (deps: {
  readonly registry: PluginRegistryApi;
  readonly maps: MapServiceApi;
}) => (input: {
  ...
  Effect.gen(function* () {
    const manifest = yield* deps.registry.getManifest(input.pluginId).pipe(
      Effect.mapError((cause) => new Error(cause instanceof Error ? cause.message : String(cause))),
    );
    const encodedManifest = Schema.encodeSync(PluginManifest)(manifest) as {
      contributes: unknown;
    };
    const contribution = findCommandContribution(encodedManifest.contributes, input.contributionId);
    ...
    if (kind === "declarative") {
      ...
      if (channel === "tileborne.maps.validate") {
        ...
        const map = yield* deps.maps.load(input.projectId, input.mapId).pipe(
          Effect.mapError((cause) => new Error(cause instanceof Error ? cause.message : String(cause))),
        );
      ...
      return yield* Effect.fail(new Error(`unsupported declarative command channel: ${channel ?? "unknown"}`));
    }

    if (contribution.entry === undefined || contribution.entry.length === 0) {
      return yield* Effect.fail(new Error(`executable command ${input.contributionId} is missing entry`));
    }
    ...
    if (input.projectId !== undefined && input.mapId !== undefined && typeof module.validateMap === "function") {
      const map = yield* deps.maps.load(input.projectId, input.mapId).pipe(
        Effect.mapError((cause) => new Error(cause instanceof Error ? cause.message : String(cause))),
      );
