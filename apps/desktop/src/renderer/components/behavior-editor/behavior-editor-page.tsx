...
import { type BehaviorTemplate, type ProjectId } from '@tileborne/core';
import {
  Badge,
  Button,
  Dialog,
...
export function focusBehaviorSourceNavigation(target: BehaviorSourceNavigationTarget): boolean {
  if (target.nodeId !== undefined) {
    const node = [...document.querySelectorAll<HTMLElement>('[data-node-id]')].find(
      (element) => element.dataset.nodeId === target.nodeId,
    );
...
export const behaviorEditorShortcut = (
  event: Pick<KeyboardEvent, 'altKey' | 'ctrlKey' | 'key' | 'metaKey' | 'shiftKey'>,
): BehaviorEditorShortcut | undefined => {
...
const rawReferences = (references: BehaviorReferenceOptions) =>
  Object.fromEntries(
...
function TemplateDialog({
  ...
  const matches = templates.filter(
    (template) =>
      query.trim().length === 0 ||
      [template.label, template.description, template.category].some((value) =>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        data-testid="behavior-template-dialog"
      >
        <DialogHeader>
          <DialogTitle>Create visual behavior</DialogTitle>
          <DialogDescription>
            Start blank or use a declarative core/plugin template. Everything remains editable.
          </DialogDescription>
        </DialogHeader>
        <label className="relative block">
          ...
          <Input
            ...
            aria-label="Search behavior templates"
            placeholder="Search templates…"
            className="pl-8"
          />
        </label>
        <div
          className="grid max-h-[58vh] grid-cols-1 gap-2 overflow-y-auto pr-1 sm:grid-cols-2"
          role="list"
        >
          <button
            type="button"
          ...
          >
            ...
            <span>
              <span className={cn('block', typography.rowTitle)}>Blank event sheet</span>
              <span className={cn('mt-1 block text-muted-foreground', typography.bodyMicro)}>
                Choose the WHEN block, conditions and actions yourself.
              </span>
            </span>
          </button>
          {matches.map((template) => (
            ...
            >
              <span className="flex size-10 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary">
                <BehaviorBlockIcon name={template.icon} className="size-5" />
              </span>
              <span className="min-w-0">
                <span className={cn('block', typography.rowTitle)}>{template.label}</span>
                <span className={cn('mt-1 block text-muted-foreground', typography.bodyMicro)}>
...
export function BehaviorResourceList({
  resources,
  selectedId,
  query,
  onQueryChange,
  ...
  const matches = useMemo(
    () =>
      resources.filter(
        ({ manifest }) =>
          query.trim().length === 0 ||
          manifest.label.toLowerCase().includes(query.trim().toLowerCase()),
      ),
    [query, resources],
  );
  ...
  const focusIndex = (index: number) => {
    ...
    requestAnimationFrame(() =>
      [...(parentRef.current?.querySelectorAll<HTMLElement>('[data-behavior-id]') ?? [])]
        .find((element) => element.dataset.behaviorId === id)
        ?.focus(),
    );
  };
  return (
    <aside
      className="flex min-h-0 flex-col border-r bg-muted/10 max-md:h-52 max-md:border-b max-md:border-r-0"
      aria-label="Behaviors"
    >
      <div className="flex items-center gap-2 border-b p-2">
        <label className="relative min-w-0 flex-1">
          <SearchIcon
            className="pointer-events-none absolute left-2 top-2.5 size-3.5 text-muted-foreground"
            aria-hidden
          />
          <Input
            value={query}
      ...
      <div
        ...
        data-testid="behavior-virtual-list"
        role="listbox"
        aria-label="Project behaviors"
      >
        {matches.length === 0 ? (
          <div className="p-5 text-center text-sm text-muted-foreground">
            {resources.length === 0 ? 'No behaviors yet.' : 'No matching behaviors.'}
          </div>
        ) : (
            ...
            {rows.map((row) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'absolute left-0 top-0 flex w-full items-center gap-2 border-b px-3 text-left hover:bg-muted/50 focus-visible:z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring',
                    active && 'bg-primary/10',
                  )}
                  style={{ height: row.size, transform: `translateY(${row.start}px)` }}
                >
                  {resource.kind === 'visual' ? (
                    <BehaviorBlockIcon name="blocks" className="size-4 shrink-0 text-primary" />
                  ) : (
                    <Code2Icon className="size-4 shrink-0 text-violet-500" aria-hidden />
                  )}
                  <span className="min-w-0 flex-1">
                    ...
                    <span className={cn('block text-muted-foreground', typography.bodyMicro)}>
                      {resource.kind === 'visual' ? 'Event sheet' : 'TypeScript'}
                    </span>
                  </span>
                </button>
              );
...
function VisualBehaviorDocument({
  projectId,
  revision,
  resource,
  useSiteCount,
  ...
  const initial = useMemo(
    () => fromBehaviorDefinition(resource.definition, resource.manifest.requiredCapabilities),
    [resource],
  );
  const [history, dispatch] = useReducer(
    reduceEditorHistory<VisualBehaviorDraft>,
    initial,
    createEditorHistory,
  );
  const [pickedReferences, setPickedReferences] = useState<BehaviorReferenceOptions>({});
  const draftReferences = useMemo(
    () => behaviorReferencesForDraft(history.present),
    [history.present],
  );
  ...
  const rememberReferenceOption = useCallback((option: BehaviorReferenceOption) => {
    ...
    setPickedReferences((current) => ({
      ...current,
      [kind]: [...(current[kind] ?? []).filter(({ id }) => id !== option.id), option],
    }));
  }, []);
  ...
  const issues = useMemo(() => {
    const validation = validateBehaviorDraft(
      history.present,
      { schemaVersion: 1, entries } as never,
      {
        ...(references.entity === undefined
          ? {}
          : { entity: new Set(references.entity.map(({ id }) => id)) }),
        ...(references.asset === undefined
          ? {}
          : { asset: new Set(references.asset.map(({ id }) => id)) }),
        ...(references.catalog === undefined
          ? {}
          : { catalog: new Set(references.catalog.map(({ id }) => id)) }),
        ...(references.behavior === undefined
          ? {}
          : { behavior: new Set(references.behavior.map(({ id }) => id)) }),
      },
    );
  ...
  const save = useCallback(async () => {
    ...
    if (issues.length > 0)
      throw new Error(
        `Fix ${issues.length} behavior problem${issues.length === 1 ? '' : 's'} before saving.`,
      );
    const requiredCapabilities = requiredCapabilitiesForDraft(history.present, {
      schemaVersion: 1,
      entries,
    } as never);
    ...
    const saved = result.snapshot.resources.find(
      ({ manifest }) => manifest.id === resource.manifest.id,
    );
    if (saved?.kind === 'visual')
      dispatch({
        type: 'reset',
        value: fromBehaviorDefinition(saved.definition, saved.manifest.requiredCapabilities),
      });
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex min-h-11 flex-wrap items-center gap-2 border-b px-3 py-1.5">
        <Badge variant={dirty ? 'secondary' : 'outline'}>
          {lifecycle?.status ?? (dirty ? 'dirty' : 'clean')}
        </Badge>
        {issues.length === 0 ? (
          <span className="flex items-center gap-1 text-xs text-emerald-600">
            <CheckIcon className="size-3.5" aria-hidden /> Valid
          </span>
        ) : (
          <span className="flex items-center gap-1 text-xs text-destructive">
            <AlertCircleIcon className="size-3.5" aria-hidden /> {issues.length} problems
          </span>
        )}
        <div className="ml-auto flex items-center gap-1">
          ...
          <Button
            ...
            size="sm"
            disabled={
              !dirty || issues.length > 0 || saveMutation.isPending || resolvedReferences.isFetching
            }
            onClick={() => void documentLifecycle.save(documentId)}
          >
            <SaveIcon className="size-3.5" aria-hidden /> Save
          </Button>
          <Button
            type="button"
            size="sm"
            ...
            title={
              dirty
                ? 'Save this event sheet before converting it.'
                : 'Convert this event sheet to native TypeScript'
            }
            onClick={() => setConvertOpen(true)}
          >
            <Code2Icon className="size-3.5" aria-hidden /> Convert to TypeScript
          </Button>
          <Button
            type="button"
            size="icon"
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <BehaviorEventSheet
          ...
          projectId={projectId}
          onReferenceOption={rememberReferenceOption}
          issues={issues}
          onChange={(value) => dispatch({ type: 'commit', value })}
        />
      </div>
      <Dialog open={convertOpen} onOpenChange={setConvertOpen}>
        <DialogContent className="max-w-lg" data-testid="behavior-convert-dialog">
          <DialogHeader>
            ...
            <DialogDescription>
              Review the irreversible canonical-source change before continuing.
            </DialogDescription>
          </DialogHeader>
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  notifyError(
                    error instanceof Error ? error.message : 'Could not convert behavior',
                  );
                }
              }}
            >
              {convertMutation.isPending ? 'Converting…' : 'Convert permanently'}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
...
export function TypeScriptBehaviorDocument({
  projectId,
  revision,
  resource,
  onSaved,
  ...
  const sourcePath =
    resource.manifest.source._tag === 'typescript' ? resource.manifest.source.sourcePath : '';
  const initialDraft = useMemo<TypeScriptBehaviorDraft>(
    () => ({
      label: resource.manifest.label,
      source: resource.source,
      exportName:
        resource.manifest.source._tag === 'typescript'
  ...
  const save = useCallback(async () => {
    ...
    if (saved?.kind === 'typescript') {
      const next = {
        ...
        source: saved.source,
        exportName:
          saved.manifest.source._tag === 'typescript'
            ? saved.manifest.source.exportName
            : 'default',
      };
      setDraft(next);
      setSavedDraft(next);
    ...
    projectId,
    resource.manifest.id,
    resource.manifest.requiredCapabilities,
    revision,
    saveMutation,
  ]);
  const lifecycle = useDocumentLifecycle({
...
export function BehaviorEditorPage() {
  const { projectId: projectIdParam } = useParams({
    from: '/editor/projects/$projectId/behaviors',
  });
  ...
  const selected =
    resources.find(({ manifest }) => String(manifest.id) === selectedId) ?? resources[0];
  useEffect(() => {
  ...
  useEffect(() => {
    if (selectedId === undefined && resources[0] !== undefined)
      setSelectedId(String(resources[0].manifest.id));
  ...
  const create = async (template?: BehaviorTemplate) => {
    ...
    try {
      const draft =
        template === undefined
          ? createBlankBehaviorDraft(registry)
          : instantiateBehaviorTemplate(template, registry, rawReferences(references));
      ...
      const created = result.snapshot.resources.find(
        ({ manifest }) => !previousIds.has(String(manifest.id)),
      );
      ...
      notifyError(error instanceof Error ? error.message : 'Could not create behavior');
    }
  };
  ...
  if (
    ...
    return (
      <div
        role="alert"
        className="m-6 rounded-md border border-destructive/40 p-4 text-sm text-destructive"
      >
        Could not open the behavior editor.{' '}
        {String(behaviorsQuery.error ?? registryQuery.error ?? '')}
      </div>
    );
  }
  return (
    <div
      className="grid h-full min-h-0 grid-cols-[minmax(14rem,18rem)_minmax(0,1fr)] max-md:grid-cols-1 max-md:grid-rows-[auto_minmax(0,1fr)]"
      data-testid="behavior-editor-page"
    >
      <BehaviorResourceList
        resources={resources}
        selectedId={selected === undefined ? undefined : String(selected.manifest.id)}
        query={query}
      ...
      <main className="flex min-h-0 min-w-0 flex-col">
        {selected === undefined ? (
          <div className="flex flex-1 items-center justify-center p-8 text-center">
            <div>
              <FilePlus2Icon className="mx-auto mb-3 size-10 text-muted-foreground" aria-hidden />
              <h1 className="text-lg font-semibold">Build gameplay visually</h1>
              <p className="mt-2 max-w-md text-sm text-muted-foreground">
                Create a WHEN / IF / DO event sheet. It compiles to the same deterministic behavior
                runtime as TypeScript.
              </p>
          ...
          <VisualBehaviorDocument
            key={String(selected.manifest.id)}
            projectId={projectId}
            ...
            onRemoved={(next) => {
              ...
              setSelectedId(
                next.resources[0] === undefined ? undefined : String(next.resources[0].manifest.id),
              );
            }}
          />
        ) : (
      ...
      <TemplateDialog
        ...
        onChoose={(template) => void create(template)}
        onBlank={() => void create()}
      />
    </div>
  );
