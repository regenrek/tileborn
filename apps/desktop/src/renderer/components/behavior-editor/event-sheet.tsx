...
const firstEntry = (
  entries: readonly BehaviorRegistryEntry[],
  kind: BehaviorRegistryEntry['kind'],
) => {
...
function ConditionNode({
  ...
  const nodeIssues = issues.filter(
    (issue) => issue.nodeId === condition.nodeId && issue.path === path,
  );
  return (
    <div
      className="space-y-2 rounded-md border border-amber-500/30 bg-amber-500/[0.04] p-3"
      data-node-id={condition.nodeId}
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="outline" className="border-amber-500/40 text-amber-600">
          {condition._tag.toUpperCase()}
        </Badge>
        <span className={cn('text-muted-foreground', typography.bodyMicro)}>IF condition</span>
        <div className="ml-auto flex items-center gap-1">
          {condition._tag === 'condition' ? (
            <>
              <Button type="button" size="sm" variant="ghost" onClick={() => wrap('all')}>
                ALL
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={() => wrap('any')}>
                ANY
              </Button>
              <Button type="button" size="sm" variant="ghost" onClick={() => wrap('not')}>
                NOT
              </Button>
            </>
          ) : null}
        ...
        <div className="space-y-2 border-l-2 border-amber-500/25 pl-3">
          ...
          <Button
            ...
            onClick={() =>
              onChange({
                ...condition,
                conditions: [...condition.conditions, newCondition(entries)],
              })
            }
          >
      ...
      {nodeIssues.map((issue) => (
        <p key={`${issue.path}:${issue.message}`} role="alert" className="text-xs text-destructive">
          {issue.message}
        </p>
      ))}
    </div>
  ...
  return (
    <div className="space-y-2" role="group" aria-label={label}>
      {actions.map((action, index) => {
        ...
        const nodeIssues = issues.filter(
          (issue) => issue.nodeId === action.nodeId && issue.path === itemPath,
        );
        return (
          <div
            key={action.nodeId}
                ...
                onChange={(invocation) =>
                  onChange(
                    actions.map((item, itemIndex) =>
                      itemIndex === index ? { ...action, invocation } : item,
                    ),
                  )
                }
              />
              ...
              <div className="space-y-3">
                <ConditionNode
                  ...
                  onChange={(condition) =>
                    onChange(
                      actions.map((item, itemIndex) =>
                        itemIndex === index ? { ...action, condition } : item,
                      ),
                    )
                  }
                />
                <fieldset className="space-y-2 rounded-md border border-border/60 p-3">
                  ...
                  <ActionList
                    ...
                    onChange={(then) =>
                      onChange(
                        actions.map((item, itemIndex) =>
                          itemIndex === index ? { ...action, then } : item,
                        ),
                      )
                    }
                  />
                ...
                <fieldset className="space-y-2 rounded-md border border-border/60 p-3">
                  <legend className={cn('px-1 text-muted-foreground', typography.rowTitle)}>
                    ELSE
                  </legend>
                  <ActionList
                    actions={action.else ?? []}
                    ...
                    onChange={(otherwise) =>
                      onChange(
                        actions.map((item, itemIndex) =>
                          itemIndex === index ? { ...action, else: otherwise } : item,
                        ),
                      )
                    }
                  />
          ...
          onAdd={(entry) =>
            onChange([
              ...actions,
              {
                _tag: 'action',
                nodeId: freshBehaviorNodeId(),
                invocation: invocationForEntry(entry),
              },
            ])
          }
        />
...
export function BehaviorEventSheet({
  draft,
  entries,
  references,
  projectId,
  ...
  const errorsFor = (prefix: string) =>
    issues.filter(({ path }) => path === prefix || path.startsWith(`${prefix}.`));
    ...
    const node =
      issue.nodeId === undefined
        ...
        : [...document.querySelectorAll<HTMLElement>('[data-node-id]')].find(
            (element) => element.dataset.nodeId === String(issue.nodeId),
          );
  ...
  return (
    <div
      ...
      data-testid="behavior-event-sheet"
    >
      {issues.length === 0 ? null : (
        <div
          role="region"
          aria-label="Behavior problems"
      ...
      <div className="grid gap-2 sm:grid-cols-[8rem_1fr] sm:items-center">
        ...
        <div data-issue-path="label">
          <Input
            id="behavior-label"
            aria-invalid={labelIssues.length > 0 ? 'true' : undefined}
            aria-describedby={labelIssues.length > 0 ? 'behavior-label-issues' : undefined}
            value={draft.label}
            onChange={(event) => onChange({ ...draft, label: event.currentTarget.value })}
          />
          {labelIssues.length === 0 ? null : (
            <p id="behavior-label-issues" role="alert" className="mt-1 text-xs text-destructive">
              {labelIssues.map(({ message }) => message).join('. ')}
            </p>
          )}
        </div>
      </div>
      ...
      <section
        className="space-y-3 rounded-lg border border-sky-500/30 bg-sky-500/[0.04] p-4"
        aria-labelledby="behavior-when-heading"
      >
        <div className="flex items-center gap-2">
          <Badge className="bg-sky-600">WHEN</Badge>
          <h2 id="behavior-when-heading" className={typography.rowTitle}>
            Trigger
          </h2>
        </div>
        <BehaviorInvocationEditor
        ...
        />
        {errorsFor('when').map((issue) => (
          <p key={`${issue.path}:${issue.message}`} className="text-xs text-destructive">
            {issue.message}
          </p>
        ))}
      </section>
      ...
      <section
        className="space-y-3 rounded-lg border border-amber-500/30 p-4"
        aria-labelledby="behavior-if-heading"
      >
        <div className="flex items-center gap-2">
          <Badge className="bg-amber-600">IF</Badge>
          <h2 id="behavior-if-heading" className={typography.rowTitle}>
            Conditions
          </h2>
          {draft.if === undefined ? null : (
            <Button
              ...
              size="sm"
              variant="ghost"
              onClick={() => onChange({ ...draft, if: undefined })}
            >
              Clear
            </Button>
          )}
        ...
        {draft.if === undefined ? (
          <div className="rounded-md border border-dashed p-5 text-center">
            <p className="mb-3 text-sm text-muted-foreground">
              No condition — every matching event runs the actions.
            </p>
            <Button
              type="button"
              size="sm"
              variant="outline"
              disabled={!entries.some(({ kind }) => kind === 'condition')}
              onClick={() => onChange({ ...draft, if: newCondition(entries) })}
            >
              <PlusIcon className="size-3.5" aria-hidden /> Add condition
            </Button>
          </div>
          ...
          <ConditionNode
            ...
            path="if"
            issues={issues}
            onChange={(nextIf) => onChange({ ...draft, if: nextIf })}
          />
        )}
        {errorsFor('if').map((issue) => (
          <p key={`${issue.path}:${issue.message}`} className="text-xs text-destructive">
            {issue.message}
          </p>
        ))}
      ...
      <section
        className="space-y-3 rounded-lg border border-emerald-500/30 p-4"
        aria-labelledby="behavior-do-heading"
      >
        <div className="flex items-center gap-2">
          <Badge className="bg-emerald-600">DO</Badge>
          <h2 id="behavior-do-heading" className={typography.rowTitle}>
            Actions in order
          </h2>
        </div>
        {draft.do.length === 0 ? (
          <p className="rounded-md border border-dashed p-5 text-center text-sm text-muted-foreground">
            Add actions or a nested branch. They execute from top to bottom.
          </p>
        ) : null}
        <ActionList
          actions={draft.do}
          ...
          onChange={(actions) => onChange({ ...draft, do: actions })}
        />
        {errorsFor('do').map((issue) => (
          <p key={`${issue.path}:${issue.message}`} className="text-xs text-destructive">
            {issue.message}
          </p>
        ))}
      ...
      <section className="space-y-3 rounded-lg border p-4" aria-labelledby="behavior-state-heading">
        <div className="flex items-center gap-2">
          ...
          <h2 id="behavior-state-heading" className={typography.rowTitle}>
            Local state
          </h2>
          <Button
            className="ml-auto"
        ...
        {draft.state.map((field, index) => (
          <div
            key={`${field.key}:${index}`}
            className="grid gap-2 rounded-md border p-2 sm:grid-cols-[1fr_1fr_1.2fr_auto] sm:items-center"
          >
            <div data-issue-path={`state.${index}.key`}>
              <Input
                aria-label="State key"
                aria-invalid={errorsFor(`state.${index}.key`).length > 0 ? 'true' : undefined}
                value={field.key}
                onChange={(event) =>
                  onChange({
                    ...draft,
                    state: draft.state.map((item, itemIndex) =>
                      itemIndex === index
                        ? new BehaviorStateField({ ...field, key: event.currentTarget.value })
                        : item,
                    ),
                  })
                }
              />
              {errorsFor(`state.${index}.key`).map((issue) => (
                <p key={issue.message} role="alert" className="mt-1 text-xs text-destructive">
                  {issue.message}
                </p>
              ))}
            </div>
            <Input
              ...
              onChange={(event) =>
                onChange({
                  ...
                  state: draft.state.map((item, itemIndex) =>
                    itemIndex === index
                      ? new BehaviorStateField({ ...field, label: event.currentTarget.value })
                      : item,
                  ),
                })
              }
            ...
            <StateValueInput
              ...
              onChange={(initialValue) =>
                onChange({
                  ...draft,
                  state: draft.state.map((item, itemIndex) =>
                    itemIndex === index ? new BehaviorStateField({ ...field, initialValue }) : item,
                  ),
                })
              }
            />
            <Button
              ...
              onClick={() =>
                onChange({
                  ...draft,
                  state: draft.state.filter((_, itemIndex) => itemIndex !== index),
                })
              }
            >
              <Trash2Icon className="size-3.5" aria-hidden />
