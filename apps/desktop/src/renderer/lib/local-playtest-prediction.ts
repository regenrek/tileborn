...
import type { InputDirection } from '@/lib/playtest-plugin-bridge';

export interface LocalMovementInput {
  readonly dir: InputDirection | undefined;
...
const LOCAL_MOVEMENT_SPEED_UNITS_PER_SECOND = 120;
...
const reduceLocalMovement: LocalTransformReducer<LocalTransform, LocalMovementInput> = (
  ...
  const distance = LOCAL_MOVEMENT_SPEED_UNITS_PER_SECOND * input.dtSeconds;
  return {
...
export class LocalPlaytestPredictionController {
  ...
  #latestInput: LocalInputPrediction | undefined;
  #pendingElapsedSeconds = 0;
  #pendingInputSequence: number | undefined;

  recordInput(input: LocalInputPrediction): void {
  ...
  project(authority: LocalAuthoritativeEntity, nowMs: number): RenderableEntity {
    ...
    const authoritativeTransform = entityTransform(entity);
...
export const applyLocalPredictionToEntities = (
  entities: readonly RenderableEntity[],
  localAuthority: LocalAuthoritativeEntity | undefined,
  controller: LocalPlaytestPredictionController,
  nowMs: number,
): RenderableEntity[] => {
