...
export function GameContentPage() {
  ...
  const activeModeId =
    typeof projectQuery.data?.project.settings?.activeGameMode === 'string'
  ...
  const visibleWeapons = useMemo(
    () =>
      (catalog?.weapons ?? []).filter(
        ({ origin, sourcePluginId }) =>
          origin === 'project' ||
          activeModeId === undefined ||
          String(sourcePluginId) === activeModeId,
      ),
  ...
  const duplicateDefinition = async (
    kind: 'object-type' | 'weapon' | 'item' | 'loot-table',
    id: string,
    name: string,
  ) => {
    try {
      ...
      if (!result.duplicated)
        return reportFailure(result.report.issues[0]?.message ?? 'Could not duplicate template');
      ...
      if (!result.removed)
        return reportFailure(
          result.blockedBy.length > 0
            ? `Still used by ${result.blockedBy.join(', ')}`
            : 'Definition is not project-owned',
        );
  ...
  const createItem = async () => {
    const visualObject = catalog?.objectTypes.find(
      ({ objectType }) => String(objectType.id) === visualEntityId,
    )?.objectType;
    const visual = visualObject === undefined ? undefined : visualFor(visualObject);
    if (
      label.trim().length === 0 ||
      visualObject === undefined ||
      visual?.placeableId === undefined ||
      Option.isNone(visual.placeableId)
    ) {
    ...
    const definition = {
      ...
      data: {
        itemKind: label
          ...
          .replace(/[^a-z0-9]+/g, '-'),
        tier: rarity,
        visualPlaceableId: visual.placeableId.value,
      },
    };
    ...
    try {
      ...
      if (!result.saved)
        return reportFailure(result.report.issues[0]?.message ?? 'Item was not saved');
      await upsertType.mutateAsync({
        projectId,
        objectTypeJson: Schema.encodeUnknownSync(GameObjectType)(pickup),
      });
  ...
  const createLootTable = async () => {
    ...
    try {
      ...
      if (!result.saved)
        return reportFailure(result.report.issues[0]?.message ?? 'Loot table was not saved');
  ...
  const createWeapon = async () => {
    ...
    const [damageValue, cooldownValue, magazineValue, reloadValue] = numeric as [
      number,
      number,
      number,
      number,
    ...
    const definition = {
      weapon: {
        id,
        damage: damageValue,
        cooldownTicks: Math.max(0, Math.round(cooldownValue)),
        magazineSize: Math.max(1, Math.round(magazineValue)),
        reloadTicks: Math.max(0, Math.round(reloadValue)),
      },
      delivery: {
    ...
    try {
      ...
      if (!result.saved)
        return reportFailure(result.report.issues[0]?.message ?? 'Weapon was not saved');
      await upsertType.mutateAsync({
        projectId,
        objectTypeJson: Schema.encodeUnknownSync(GameObjectType)(weaponEntity),
      });
  ...
  return (
    <ScrollArea className="h-full">
      <main
        ...
        data-testid="game-content-page"
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="text-xl font-semibold">Gameplay content</h1>
            <p className="text-sm text-muted-foreground">
              Create match-ready objects, weapons, pickups, items and loot without editing JSON or
              IDs.
            </p>
          </div>
        ...
        {tab === 'objects' ? (
          <section className="grid gap-2 sm:grid-cols-2">
            {catalog?.objectTypes.map(({ objectType, origin }) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">{objectType.label}</p>
                  <Badge variant="secondary">{origin === 'plugin' ? 'Template' : 'Project'}</Badge>
                </div>
                {origin === 'plugin' ? (
                  <Button
                    size="icon-sm"
                    ...
                    aria-label={`Duplicate ${objectType.label}`}
                  >
                    <CopyIcon />
                  </Button>
                ) : null}
            ...
            <Link
              ...
              params={{ projectId }}
              className="rounded-md border border-dashed p-4 text-sm hover:bg-muted/50"
            >
              <PlusIcon className="mb-2 size-4" />
              Create or edit a placeable object with the real sprite picker
            </Link>
          </section>
        ) : null}
        ...
        {tab !== 'objects' ? (
          <section className="rounded-md border p-4" data-testid="content-create-form">
            <h2 className={cn('mb-3 text-sm font-semibold')}>
              Create {tab === 'loot' ? 'loot table' : tab.slice(0, -1)}
            </h2>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="space-y-1">
                <span className={typography.rowMeta}>Name</span>
                <Input
                  value={label}
              ...
              {tab === 'items' ? (
                <label className="space-y-1">
                  <span className={typography.rowMeta}>Category</span>
                  <Input value={category} onChange={(event) => setCategory(event.target.value)} />
                </label>
              ) : null}
              {tab === 'items' || tab === 'loot' ? (
                <label className="space-y-1">
                  <span className={typography.rowMeta}>Rarity / tier</span>
                  <select
                    className="h-9 w-full rounded-md border bg-background px-2 text-sm"
                    value={rarity}
                    onChange={(event) => setRarity(event.target.value as typeof rarity)}
                    data-testid="content-rarity"
                  >
              ...
              {tab === 'items' || tab === 'weapons' ? (
                <label className="space-y-1">
                  <span className={typography.rowMeta}>Project visual / pickup entity</span>
                  <select
                    className="h-9 w-full rounded-md border bg-background px-2 text-sm"
                    value={visualEntityId}
                    onChange={(event) => setVisualEntityId(event.target.value)}
                    data-testid="content-visual-entity"
                  >
                    <option value="">Choose by name…</option>
                    {visualObjects.map(({ objectType }) => (
                      <option key={String(objectType.id)} value={String(objectType.id)}>
                        {objectType.label}
                      </option>
                    ))}
                  </select>
                  {visualObjects.length === 0 ? (
                    <span className={typography.rowMeta}>
                      Duplicate a visual template in Objects first, or create one in the Entity
                      Editor.
                    </span>
                  ) : null}
                </label>
              ...
              {tab === 'loot' ? (
                <>
                  <label className="space-y-1">
                    <span className={typography.rowMeta}>Item</span>
                    <select
                      className="h-9 w-full rounded-md border bg-background px-2 text-sm"
                      value={selectedItemId}
                      onChange={(event) => setSelectedItemId(event.target.value)}
                      data-testid="content-loot-item"
                    >
                      <option value="">Choose by name…</option>
                  ...
                  <label className="space-y-1">
                    <span className={typography.rowMeta}>Drop weight</span>
                    <Input
                      type="number"
                      min={0.1}
                      step={0.1}
              ...
              {tab === 'weapons' ? (
                <>
                  {[
                    ...
                    ['Cooldown ticks', cooldown, setCooldown],
                    ['Magazine', magazine, setMagazine],
                    ['Reload ticks', reload, setReload],
                  ].map(([name, value, setter]) => (
                    <label key={name as string} className="space-y-1">
                      <span className={typography.rowMeta}>{name as string}</span>
                      <Input
                        type="number"
                        min={0}
        ...
        {tab === 'items' ? (
          <DefinitionList
            entries={
              catalog?.items.map((item) => ({ id: String(item.id), label: item.label })) ?? []
            }
            isProject={isProject}
            busy={busy}
            duplicate={(id, name) => duplicateDefinition('item', id, name)}
            remove={(id) => removeDefinition('item', id)}
          />
        ) : null}
        {tab === 'loot' ? (
          <DefinitionList
            entries={
              catalog?.lootTables.map((table) => ({ id: String(table.id), label: table.label })) ??
              []
            }
            isProject={isProject}
            busy={busy}
            duplicate={(id, name) => duplicateDefinition('loot-table', id, name)}
            remove={(id) => removeDefinition('loot-table', id)}
          />
        ) : null}
        {tab === 'weapons' ? (
          <DefinitionList
            entries={visibleWeapons.map(({ entry, label: name }) => {
              const visualObject = catalog?.objectTypes.find(({ objectType }) =>
                objectType.components.some(
                  (component) =>
                    component._tag === 'weapon-ref' &&
                    String(component.weaponId) === String(entry.weapon.id),
                ),
              )?.objectType;
              return {
                id: String(entry.weapon.id),
                label: name,
                ...(visualObject === undefined ? {} : { visualObject }),
              };
            })}
            isProject={isProject}
            busy={busy}
            duplicate={(id, name) => duplicateDefinition('weapon', id, name)}
            remove={(id) => removeDefinition('weapon', id)}
          />
        ) : null}
      </main>
    </ScrollArea>
...
function DefinitionList({
  ...
  readonly entries: readonly { id: string; label: string; visualObject?: GameObjectType }[];
  readonly isProject: (id: string) => boolean | undefined;
  readonly busy: boolean;
  readonly duplicate: (id: string, label: string) => Promise<unknown>;
  readonly remove: (id: string) => Promise<unknown>;
}) {
  return (
    <ul className="grid gap-2 sm:grid-cols-2">
      {entries.map((entry) => {
        const projectOwned = isProject(entry.id);
        return (
          <li key={entry.id} className="flex items-center gap-2 rounded-md border p-3">
            {entry.visualObject === undefined ? null : (
              <ObjectThumbnail objectType={entry.visualObject} />
            )}
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium">{entry.label}</p>
              <Badge variant="secondary">
                {projectOwned ? 'Project override' : 'Immutable template'}
              </Badge>
            </div>
            {projectOwned ? (
              <Button
              ...
              <Button
                ...
                variant="ghost"
                disabled={busy}
                onClick={() => void duplicate(entry.id, entry.label)}
