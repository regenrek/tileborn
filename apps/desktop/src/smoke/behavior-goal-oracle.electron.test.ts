...
describe('live behavior Goal Oracle (fresh-profile Electron)', () => {
  ...
  afterAll(async () => {
    ...
    context = undefined;
    await Promise.all(isolatedArtifacts.map((directory) => rm(directory, { recursive: true, force: true })));
  });
  ...
  it('authors equivalent visual and TypeScript behavior, debugs it, reopens it, and boots the shipped copy', async () => {
    ...
    const authored = await page.evaluate(async ({ projectId, visualNode, missingNode, missingBehavior }) => {
      ...
      const createVisual = async (label: string, definition: ReturnType<typeof visualDraft>) => {
        ...
        const beforeIds = new Set(before.snapshot.resources.map(({ manifest }) => String(manifest.id)));
        ...
        const resource = result.snapshot.resources.find(({ manifest }) => !beforeIds.has(String(manifest.id)));
        if (resource === undefined) throw new Error(`Could not identify created behavior ${label}`);
      ...
      if (nativeResource?.kind !== 'typescript') throw new Error('Native proof conversion failed');
...
export default defineBehavior({
  ...
  on: { 'runtime.tick': ({ state }) => state.set('proof', true) },
      ...
      if (runawayResource?.kind !== 'typescript') throw new Error('Runaway proof conversion failed');
...
export default defineBehavior({
  ...
  on: { 'runtime.tick': ({ state }) => state.set('proof', true) },
      ...
      const missingDiagnostic = broken.report.diagnostics.find(
        ({ code, behaviorId }) => code === 'behavior.reference-missing' && behaviorId === missing.resource.manifest.id,
      );
      if (missingDiagnostic?.navigation?.kind !== 'behavior') {
        throw new Error(`Missing reference was not actionable: ${JSON.stringify(broken.report.diagnostics)}`);
      }
      const missingResource = missing.snapshot.resources.find(
        ({ manifest }) => manifest.id === missing.resource.manifest.id,
      );
      if (missingResource?.kind !== 'visual') throw new Error('Missing reference visual resource disappeared');
      const repairedDefinition = {
        ...missingResource.definition,
        do: missingResource.definition.do.map((node) => node.nodeId === missingNode && node._tag === 'action'
          ? {
              ...node,
              invocation: {
                ...node.invocation,
      ...
      if (!ready.report.ok) throw new Error(`Reference repair did not restore readiness: ${JSON.stringify(ready.report.diagnostics)}`);
    ...
    await expect.poll(async () => page.evaluate(async (projectId) => {
      ...
      return sessions.some((entry) => String(entry.projectId) === projectId && entry.status === 'Running');
    }, created.projectId), { timeout: 30_000, intervals: [100, 250, 500] }).toBe(true);
    const session = await page.evaluate(async (projectId) => {
      const { sessions } = await window.tileborne.playtest.list({});
      const running = sessions.find((entry) => String(entry.projectId) === projectId && entry.status === 'Running');
    ...
    }, session.id), { timeout: 30_000, intervals: [100, 250, 500] }).toBe(true);

    const equivalent = await expect.poll(async () => page.evaluate(async ({ sessionId, visualId, nativeId }) => {
      const { snapshot } = await window.tileborne.playtest.behaviorDebugInspect({ sessionId });
    ...
    const paused = await page.evaluate(async (sessionId) => (
      await window.tileborne.playtest.behaviorDebugControl({ sessionId, command: 'pause' })
    ).snapshot, session.id);
    expect(paused.status).toBe('paused');
    const stepped = await page.evaluate(async (sessionId) => (
    ...
    const invalidReload = await page.evaluate(async ({ projectId, nativeId }) => {
      const opened = await window.tileborne.behaviors.open({ projectId });
      const resource = opened.snapshot.resources.find(({ manifest }) => String(manifest.id) === nativeId);
    ...
    }, session.id), { timeout: 15_000 }).toMatchObject({
    ...
    await page.evaluate(async ({ projectId, nativeId, source }) => {
      const opened = await window.tileborne.behaviors.open({ projectId });
      const resource = opened.snapshot.resources.find(({ manifest }) => String(manifest.id) === nativeId);
      if (resource?.kind !== 'typescript') throw new Error('Native proof resource missing');
    ...
    await expect.poll(async () => page.evaluate(async (sessionId) => (
      await window.tileborne.playtest.behaviorDebugInspect({ sessionId })
    ).snapshot.lastReload?.status, session.id), { timeout: 15_000 }).toBe('applied');

    await page.evaluate(async ({ projectId, runawayId, source }) => {
      const opened = await window.tileborne.behaviors.open({ projectId });
      const resource = opened.snapshot.resources.find(({ manifest }) => String(manifest.id) === runawayId);
    ...
    }, session.id), { timeout: 30_000, intervals: [100, 250, 500] }).toContain('Behavior worker exceeded');
    ...
    page = context.page;
    const reopened = await page.evaluate(async ({ projectId, ids }) => {
      const { snapshot } = await window.tileborne.behaviors.open({ projectId });
      return {
        revision: snapshot.revision,
        resources: snapshot.resources
          .filter(({ manifest }) => ids.includes(String(manifest.id)))
          .map((resource) => ({ id: String(resource.manifest.id), kind: resource.kind, label: resource.manifest.label })),
      };
    }, {
      projectId: created.projectId,
    ...
    expect(reopened.resources.map(({ kind }) => kind).sort()).toEqual(['typescript', 'typescript', 'visual', 'visual']);
    ...
    await expect.poll(async () => page.evaluate(async () => {
      ...
      return jobs.find(({ status, result }) => status !== 'Completed' || result !== undefined)?.id;
    }).then((value) => {
      jobId = value;
      return value;
    }), { timeout: 15_000 }).toMatch(/^job:/);
    ...
    const artifact = await page.evaluate(async (startupMapId) => {
      ...
      const result = jobs
        ...
        .map(({ result }) => result)
        .find((value) => typeof value === 'object' && value !== null && 'startupMapId' in value && value.startupMapId === startupMapId);
    ...
    try {
      ...
      const room = await host.fetch('/rooms/create', {
        ...
        body: JSON.stringify({ mapId: created.mapId, options: { idempotencyKey: 'behavior-goal-oracle-copy' } }),
      });
