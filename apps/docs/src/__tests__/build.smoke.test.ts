...
describe("docs build output", () => {
  ...
  it("publishes executable plugin/SDK examples without stale manifest or CLI contracts", () => {
    ...
    expect(plugins).not.toMatch(/"engineRange"|"contributions"|tileborne plugin add|tileborne plugin validate/u);
    ...
    expect(fs.existsSync(path.join(docsAppRoot, "../..", "packages/game-sdk/examples/open-exit.ts"))).toBe(true);
    expect(fs.existsSync(path.join(docsAppRoot, "../..", "packages/game-sdk/examples/plugin-event.ts"))).toBe(true);
    expect(fs.existsSync(path.join(docsAppRoot, "../..", "packages/plugin-battle-royale/src/runtime-adapter.ts"))).toBe(true);
    expect(fs.existsSync(path.join(docsAppRoot, "../..", "packages/plugin-example-arena/src/runtime-adapter.ts"))).toBe(true);
    expect(fs.existsSync(path.join(docsAppRoot, "../..", "packages/plugin-api/examples/runtime-adapter.ts"))).toBe(true);
    expect(fs.existsSync(path.join(docsAppRoot, "../..", "packages/plugin-api/examples/tileborne-plugin.json"))).toBe(true);
  });
});
