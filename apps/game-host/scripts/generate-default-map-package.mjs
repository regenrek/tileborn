...
/**
 * Generate the dev-room default `RuntimeMapPackage` module from the REAL
 * sources instead of hand-baked data (ADR-0030): the BR plugin's shipped
 * game-object catalog, the real `exportBattleRoyaleModeData` exporter, the
 * plugin's default player models, and a small authored 64x64 dev arena map.
 * The assembled wire JSON is validated against the `RuntimeMapPackage`
 * schema before the TS module is written.
 *
...
export const generateDefaultMapPackage = async () => {
  ...
  await writeFile(
    path.join(generatedDir, "default-map-package.ts"),
    `/**
 * GENERATED by scripts/generate-default-map-package.mjs — do not edit.
 *
 * Encoded \`RuntimeMapPackage\` (ADR-0030) used by the package-less dev room:
 * built from the BR plugin's shipped game-object catalog, the real
 * \`exportBattleRoyaleModeData\` output, the plugin's default player models,
 * and a small authored 64x64 dev arena map.
 */
export const defaultMapPackage: unknown = ${JSON.stringify(wire, null, 2)};
`,
    "utf8",
  );
