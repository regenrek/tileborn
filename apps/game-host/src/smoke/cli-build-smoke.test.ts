...
describe("game-host smoke — CLI cloudflare build pipeline", () => {
  ...
  it("builds a deployable worker bundle and passes health + discover in Miniflare", async () => {
    ...
    const buildPayload = JSON.parse(String(build.stdout)) as {
      readonly ok: boolean;
      readonly data: { readonly bundlePath: string; readonly files: readonly string[]; readonly manifestHash: string };
    };
    ...
    const manifest = JSON.parse(await readFile(path.join(outputDir, "manifest.json"), "utf8")) as BundledManifest;
