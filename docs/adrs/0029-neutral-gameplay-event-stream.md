...
- **Transport.** Events ride the existing snapshot/frame channel (a `gameplayEvents` array on the delta frame / runtime HUD payload) — **no new IPC channel**. `PlaytestRuntimeHudEvent` is superseded by (re-expressed as a projection of) `GameplayEvent`; pre-release hard-cut, no dual vocabulary.
