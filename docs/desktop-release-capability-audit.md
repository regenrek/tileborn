...
desktop signing, desktop update, or desktop recovery capability.
...
> still **NO-GO** because no candidate DMG, native install/relaunch/recovery run, approved Team ID,
> publish approval, or active scoped credential is present. Use
...
| Desktop updates                           | The macOS arm64 signed A-to-B update path now has candidate-only oracle coverage; no release channel, feed publication, or operator-approved rollout exists yet.                                                                                                                                                                                                              | `apps/desktop/src/main` for lifecycle/UX and the desktop release pipeline for feeds/artifacts.                                                                            | Keep the updater candidate-gated until a signed ZIP, feed, Restart/Later UX, relaunch oracle, and publication contract all pass under release approval.                                                      | **Candidate-only for macOS arm64; not an available release channel.** Manual replacement with a complete signed installer remains the upgrade path until publication approval.               | A future feed needs release-host credentials, maintainer approval, and a signing continuity policy.                                                                                                                    |
...
| Project-content recovery                  | Project schema migration and transactional save/recovery protect project data. Desktop release verification treats removing and installing the current app again as recovery-only behavior, not a previous-version rollback guarantee.                                                                                                                                        | Desktop release runbook plus `@tileborne/core` schema compatibility and `@tileborne/services-app` durable project owner.                                                  | Keep relaunch persistence evidence attached to install/relaunch verification and do not claim previous-version downgrade support until the candidate automatic-update path graduates to a supported channel. | **Required relaunch persistence evidence for the macOS arm64 candidate.** Desktop rollback remains out of scope for 1.0.                                                                     | No private runtime credential. Any destructive manual repair requires explicit user confirmation and preserved project data.                                                                                           |
...
5. define manual replacement and recovery, while marking automatic updates candidate-only and remote crash
