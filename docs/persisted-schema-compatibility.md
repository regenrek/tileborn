...
| ---------------------------------------------- | ------------: | ----------------------------- | ---------------------------- | -------------------------- | --------------------------------------------------------------------------------- |
| Project manifest                               |             1 | 0, 1                          | migrate missing-version v0   | refuse / refuse            | source; core `ProjectManifest`, services-app backup-first `projectMigrationChain` |
...
claim testable. Project-manifest upgrades now preserve an exact-byte,
content-addressed v0 source backup under the project root, verify it before the
first source write, and expose a containment-checked restore owner. Future and
corrupt manifests are refused before backup or mutation. It does **not** claim
migration support that does not exist. The remaining gaps are:

1. further schema-version migrations beyond the committed project v0→v1 chain
   and the current-version map/content shape migrations;
2. runtime validation for room state and the bundled game manifest;
3. explicit envelopes for generic asset packs, thin-game config, job records,
   model/loadout selections, and build/export metadata;
4. plugin-lock identity/version enforcement and activation of the plugin
