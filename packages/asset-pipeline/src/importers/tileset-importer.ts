...
import { deterministicAssetId, deterministicTileId, deterministicTileSetId } from "./deterministic-id.js";
...
export const tilesetImporter: AssetImporter = {
  ...
  import: (input) => {
    ...
    const imageAssetId = deterministicAssetId(`${tilesetImporter.id}:image:${input.filename}:${hash}`);
