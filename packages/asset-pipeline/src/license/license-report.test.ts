...
import { AssetPackManifest, AssetPackManifestAsset, assetsFromManifest } from "../pack/pack-manifest.js";
import {
...
describe("license report", () => {
  ...
  it("formats plain text reports", () => {
    ...
    expect(formatLicenseReportPlain(summarizeLicenses(indexPack(pack, assetsFromManifest(pack))))).toContain(
      "CC0-1.0: 1",
    );
  });
  ...
  it("formats markdown with attribution links", () => {
    ...
    const markdown = formatLicenseReportMarkdown(summarizeLicenses(indexPack(pack, assetsFromManifest(pack))));
