...
describe('pack merge', () => {
  ...
  it('surfaces asset id collisions without overriding', () => {
    ...
    expect(merged.assetsById.get(shared)?.map((entry) => entry.path)).toEqual([
      'first.png',
      'second.png',
    ]);
  });
  ...
  it('resolves collisions by preferring first deterministic pack', () => {
    ...
    expect(resolveConflicts(merged, new PreferFirst({})).assets.map((entry) => entry.path)).toEqual(
      ['first.png'],
    );
  });
  ...
  it('resolves collisions by preferring last deterministic pack', () => {
    ...
    expect(resolveConflicts(merged, new PreferLast({})).assets.map((entry) => entry.path)).toEqual([
      'second.png',
    ]);
  });
