...
const ALLOW_LIST: readonly ImportAllowEntry[] = [
  /** Allow (deferred fu-core-node-crypto): core hashing still uses Node crypto until Web Crypto migration */
  {
    ...
    reason: "deferred: migrate core hashing to Web Crypto/subtle (see docs/follow-ups.md fu-core-node-crypto)",
  },
  /** Allow (deferred fu-runtime-node-asset-loader): runtime asset loader reads bytes from disk in Node harness */
  {
    fileSuffix: "packages/runtime/src/assets/runtime-asset-loader.ts",
    line: 133,
    moduleSpecifier: "node:fs/promises",
    reason: "deferred: inject filesystem port for renderer-safe asset loading (see docs/follow-ups.md fu-runtime-node-asset-loader)",
  },
  /** Allow (deferred fu-runtime-renderer-test-node): renderer integration tests use temp dirs via Node APIs */
  {
    ...
    line: 4,
    moduleSpecifier: "node:fs/promises",
    reason: "deferred: move renderer tests behind platform test harness (see docs/follow-ups.md fu-runtime-renderer-test-node)",
  },
  {
    fileSuffix: "packages/runtime/src/renderer/renderer.test.ts",
    line: 5,
...
const NATIVE_MODULE_PREFIXES = ["better-sqlite3", "sharp", "canvas", "node-gyp", "fsevents"] as const;

const REACT_FORBIDDEN_MODULES = new Set(["react", "react-dom"]);

const REACT_FORBIDDEN_PREFIXES = ["@tanstack/react-"] as const;

const RENDERER_PLUGIN_EXECUTABLE_PREFIXES = ["@tileborne/services-plugin", "@tileborne-plugins/"] as const;
...
const scanDirectory = (
  rootDir: string,
  rules: Array<(file: string, line: number, moduleSpecifier: string) => ImportViolation | undefined>,
): ImportViolation[] => {
...
const rendererPluginRule = (file: string, line: number, moduleSpecifier: string): ImportViolation | undefined => {
...
describe("import boundaries", () => {
  it("packages/ui, packages/core, packages/runtime, and packages/sdk-tileset avoid Node/Electron/native imports", () => {
    ...
    const message = violations
      .map((violation) => `${violation.rule}: ${violation.file}:${violation.line} imports "${violation.moduleSpecifier}"`)
      .join("\n");
  ...
  it("packages/sdk-tileset depends only on @tileborne/core, effect, and fast-xml-parser", () => {
    ...
    const forbidden = dependencies.filter((dependency) => !SDK_TILESET_ALLOWED_DEPENDENCIES.has(dependency));
