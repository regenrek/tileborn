...
describe.sequential('config e2e', () => {
  ...
  it('config set/get round-trips lastOpenedProject', async () => {
    ...
    const got = await expectCliJsonData<{
      readonly key: string;
      readonly value: { readonly _tag: string; readonly value: string };
    }>(['config', 'get', 'lastOpenedProject']);
  ...
  it('config list --json includes configured keys', async () => {
    ...
    const list = await expectCliJsonData<{
      readonly lastOpenedProject: { readonly _tag: string; readonly value: string };
    }>(['config', 'list']);
