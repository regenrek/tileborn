...
describe.sequential("map e2e", () => {
  ...
  const seedProject = async (): Promise<{ readonly projectSlug: string; readonly projectPath: string }> => {
  ...
  it("map export writes parseable canonical json", async () => {
    ...
    const exported = JSON.parse(await readFile(path.join(projectPath, outFile), "utf8")) as { readonly id: string };
