...
describe.sequential("plugin e2e", () => {
  ...
  it("plugin create scaffolds a plugin directory", async () => {
    ...
    process.chdir(cwd);
    try {
      const data = await expectCliJsonData<{ readonly directory: string; readonly manifest: { readonly id: string } }>([
        "plugin",
        "create",
        "my-plug",
      ]);
      expect(data.manifest.id).toBe("@tileborne-plugins/my-plug");
      await expect(readFile(path.join(cwd, "my-plug", "tileborne-plugin.json"), "utf8")).resolves.toContain(
        "@tileborne-plugins/my-plug",
      );
    } finally {
  ...
  it("plugin pack writes .tbpack and integrity metadata", async () => {
    ...
    const data = await expectCliJsonData<{ readonly archivePath: string; readonly integrity: string }>([
      "plugin",
      "pack",
      source,
      "--out",
    ...
    await expect(readFile(`${data.archivePath}.meta.json`, "utf8")).resolves.toContain(data.integrity);
  });
  ...
  it("plugin list --json shows the installed plugin", async () => {
    ...
    expect(data.plugins.find((entry) => entry.id === "@tileborne-plugins/e2e-list")?.integrityOk).toBe(true);
  });
});
