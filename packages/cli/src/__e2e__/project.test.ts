...
describe.sequential('project e2e', () => {
  ...
  it('project info --json returns slug and uuid after init', async () => {
    ...
    const init = await expectCliJsonData<{
      readonly manifest: { readonly id: string; readonly name: string };
    }>(['project', 'init', 'demo-info', '--here'], { cwd: projectDir });
    const info = await expectCliJsonData<{
      readonly manifest: { readonly id: string; readonly name: string };
    }>(['project', 'info'], { cwd: projectDir });
  ...
  it('project upgrade --json is idempotent on a fresh project', async () => {
    ...
    const upgrade = await expectCliJsonData<{
      readonly changed: boolean;
      readonly fromVersion: number;
    }>(['project', 'upgrade'], { cwd: projectDir });
