...
describe.sequential('CLI spec §12 sweep e2e', () => {
  ...
  it('chains doctor → project init → asset import → map generate → playtest in tmp HOME', async () => {
    ...
    const packManifest = JSON.parse(
      await readFile(path.join(sampleAssetPackFixture, 'tileborne-asset-pack.json'), 'utf8'),
    ) as {
