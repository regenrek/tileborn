...
const readPluginArg = (args: Record<string, unknown>): string | undefined =>
  readStringArg(args, 'plugin');
  ...
  subCommands: {
    ...
    build: {
      ...
      args: {
        ...
        project: {
          type: 'string' as const,
          description:
            'Project slug: assemble its maps as runtime map packages into the build (cloudflare target)',
          required: false,
        },
        map: {
          type: 'string' as const,
      ...
      async run(context: CliRunContext) {
        await runCliCommand(
          ...
          Effect.gen(function* () {
            ...
            const target = Schema.decodeUnknownSync(BuildTarget)(
              readStringArg(context.args, 'target') ?? 'local',
            );
    ...
    serve: {
      ...
      args: {
        ...
        'signing-key': {
          type: 'string' as const,
          description: 'Handoff signing key',
          required: false,
        },
        ...
        dir: {
          type: 'string' as const,
          description:
            'Built game artifact directory to serve (from `game build --target local`); default: the dev game-host bundle',
          required: false,
        },
      },
      async run(context: CliRunContext) {
