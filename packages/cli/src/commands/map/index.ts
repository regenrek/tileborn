...
export const mapCommand = {
  ...
  subCommands: {
        ...
        await runCliCommand(
          ...
          Effect.gen(function* () {
            ...
            if (format !== "json" && format !== "tiled") {
              yield* Effect.fail(new CliValidationError({ message: "format must be json or tiled" }));
            }
            return yield* maps.exportToFile(projectId, mapId, format as "json" | "tiled", out as string);
          }),
        );
      },
    },
    ...
    generate: {
      ...
      args: {
        ...globalArgs,
        slug: { type: "positional" as const, description: "Map slug stored in properties (defaults to --preset)", required: false },
        ...
        template: { type: "string" as const, description: "empty|grid|biome (alias: --preset)", default: "empty" },
    ...
    "import-tiled": {
      meta: { name: "import-tiled", description: "Import a Tiled TMJ or TMX map into the active project" },
      args: {
        ...globalArgs,
        file: { type: "positional" as const, description: "Tiled JSON or TMX file", required: true },
        profile: { type: "string" as const, description: "standard|standard-plus-hints", default: "standard" },
        project: { type: "string" as const, description: "Project slug", required: false },
      },
      async run(context: CliRunContext) {
        await runCliCommand(
