...
import { globalArgs, readBooleanArg, readGlobalCliArgs, readStringArg, renderContextFromArgs, type CliRunContext } from "../shared.js";
...
export const playtestCommand = {
  ...
  args: {
    ...
    artifact: { type: "string" as const, description: "Artifact output directory", required: false },
    duration: { type: "string" as const, description: "Headless duration in seconds", default: "5" },
    open: { type: "boolean" as const, description: "Open desktop client via tileborne:// deep link", default: false },
    multiplayer: { type: "boolean" as const, description: "Boot local miniflare game-host and create a room", default: false },
    port: { type: "string" as const, description: "Local game-host port (0 = auto)", default: "8787" },
    ...
    await runCliCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!Number.isFinite(durationSec) || durationSec <= 0) {
          yield* Effect.fail(new CliValidationError({ message: "duration must be a positive number" }));
        }
