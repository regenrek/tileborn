    ...
    install: {
      ...
      args: {
        ...
        local: {
          ...
          description: 'Install from a local directory or archive',
          required: false,
        },
        tarball: {
          type: 'string' as const,
          description: 'Install from a tarball path or URL',
          required: false,
        },
        integrity: {
          type: 'string' as const,
          description: 'Expected sha256 integrity for tarball',
          required: false,
        },
        git: {
          type: 'string' as const,
          description: 'Install from a git repository URL',
          required: false,
        },
        ref: {
          type: 'string' as const,
        ...
        'dev-symlink': {
          type: 'string' as const,
          description: 'Install via dev symlink',
          required: false,
        },
      },
          ...
          Effect.gen(function* () {
            ...
            if (!verified?.ok) {
              return yield* Effect.fail(
                new Error(verified?.message ?? `plugin not found: ${pluginId}`),
              );
            }
      ...
      async run(context: CliRunContext) {
        ...
        try {
          ...
          const message = failed
            .map((entry) => `${entry.pluginId}: ${entry.message ?? 'integrity check failed'}`)
            .join('; ');
      ...
      async run(context: CliRunContext) {
        ...
        if (!name) {
          renderFailure(
            ...
            new Error('plugin name is required'),
            ExitCode.Usage,
          );
          return;
        }
        await runCliCommand(
          ...
          Effect.gen(function* () {
            const installer = yield* PluginInstallerService;
            return yield* installer.create(
              name,
              readStringArg(context.args, 'template'),
              process.cwd(),
            );
          }),
        );
    ...
    link: {
      ...
      async run(context: CliRunContext) {
        ...
        if (!linkPath) {
          renderFailure(
            renderContextFromArgs(readGlobalCliArgs(context.args)),
            new Error('plugin link requires a path'),
            ExitCode.Usage,
          );
          return;
        }
    ...
    pack: {
      ...
      async run(context: CliRunContext) {
        ...
        if (!sourcePath) {
          renderFailure(
            renderContextFromArgs(readGlobalCliArgs(context.args)),
            new Error('plugin pack requires a directory path'),
            ExitCode.Usage,
          );
          return;
        }
