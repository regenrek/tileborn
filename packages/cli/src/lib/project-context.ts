...
const resolveExplicitProjectId = (
  ...
  Effect.gen(function* () {
    ...
    const listed = summaries.find(
      (entry) => entry.name === projectSlug || entry.id === projectSlug,
    );
    if (listed) {
      return listed.id;
    }
    ...
    if (openedId) {
      ...
      const openedFromCwd = yield* Effect.promise(() =>
        findProjectInAncestors(cwd, openedId as ProjectId),
      );
      if (
        openedFromCwd &&
        (openedFromCwd.name === projectSlug || openedFromCwd.id === projectSlug)
      ) {
  ...
  Effect.gen(function* () {
    ...
    if (!openedId) {
      return yield* Effect.fail(
        new CliUsageError({
          message:
            'project is required (pass --project, run from a project directory, or set lastOpenedProject in config)',
        }),
      );
    }
