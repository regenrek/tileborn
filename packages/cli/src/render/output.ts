...
export const renderInfo = (
  ...
  if (ctx.json) {
    process.stdout.write(
      `${JSON.stringify({ ok: true, message, ...(data ? { data } : {}) }, null, 2)}\n`,
    );
    return;
  }
  if (data) {
...
export const renderMultiplayerStatus = (
  ctx: RenderContext,
  payload: MultiplayerStatusPayload,
): void => {
  if (ctx.json) {
    process.stdout.write(`${JSON.stringify({ ok: true, data: payload }, null, 2)}\n`);
    return;
...
export const renderGameServeStatus = (
  ctx: RenderContext,
  payload: GameServeStatusPayload,
): void => {
  if (ctx.json) {
    process.stdout.write(`${JSON.stringify({ ok: true, data: payload }, null, 2)}\n`);
    return;
