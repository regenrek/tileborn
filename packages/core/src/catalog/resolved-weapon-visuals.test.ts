...
describe('deriveWeaponVisuals', () => {
  ...
  it('round-trips ResolvedWeaponVisuals through its schema (artifact bake shape)', () => {
    const weapon = entity('test-rifle', [visualRef(), { _tag: 'weapon-ref', weaponId: WEAPON_ID }]);
  ...
  it('reports issues for missing companions and render-incomplete entities', () => {
    ...
    expect(
      issues.some(
        (issue) => issue.path === 'weapon-ref.projectileEntityId' && issue.message.includes(ghost),
      ),
    ).toBe(true);
  });
