...
describe("RawTrigger union", () => {
  it("decodes each trigger variant by its _tag", () => {
    expect(Schema.decodeUnknownSync(RawTrigger)({ _tag: "key", code: "KeyW" })).toBeInstanceOf(KeyTrigger);
    expect(Schema.decodeUnknownSync(RawTrigger)({ _tag: "mouseButton", button: 1 })).toBeInstanceOf(
      MouseButtonTrigger,
    );
    expect(Schema.decodeUnknownSync(RawTrigger)({ _tag: "axis", axis: 0, sign: -1 })._tag).toBe("axis");
