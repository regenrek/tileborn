...
The public worker-safe adapter interfaces are exported as `RuntimeAdapterHost`, `RuntimeAdapterWorld`, `RuntimeAdapter`, and `CreateRuntimeAdapter`. [The compile-checked example](./examples/runtime-adapter.ts) and its companion [manifest fixture](./examples/tileborne-plugin.json) document the published pair; the package test suite exercises the named runtime factory without crossing into the filesystem loader layer.
