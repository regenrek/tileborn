...
describe("materializeGameSettingsForm + value policy", () => {
  ...
  it("rejects a draft that is non-finite or out of declared bounds", () => {
    const form = materializeGameSettingsForm(decodeOrThrow(validData));
    expect(parseGameSettingsDraft(form, { maxPlayers: "x", damagePerSecOutside: "5" })).toBeUndefined();
    expect(parseGameSettingsDraft(form, { maxPlayers: "0", damagePerSecOutside: "5" })).toBeUndefined();
    expect(parseGameSettingsDraft(form, { maxPlayers: "8", damagePerSecOutside: "999" })).toBeUndefined();
  });
});
