...
const clipRows = {
  ...
  shoot: {
    ...
    frames: [0, 1, 2, 3, 4, 5],
    durationMs: 70,
    fallbackNote: "Codex-pet Maltipoo atlas has no gun-fire row; BR weapon overlay and muzzle flash own shooting.",
  },
  reload: {
    sourceState: "waiting",
    row: 6,
    frames: [0, 1, 2, 3, 4, 5],
    durationMs: 120,
    fallbackNote: "Codex-pet Maltipoo atlas has no reload row; waiting loop is the visible reload fallback.",
  },
...
const PLAYER_MODELS = [
  {
    ...
    clipBase: 0x1000,
    tileBase: 0x3000,
    upstreamSourcePath: path.join(petwarsUprightMaltipooDir, "maltipoo-mae", "final", "spritesheet.png"),
    upstreamValidationPath: path.join(petwarsUprightMaltipooDir, "maltipoo-mae", "final", "validation.json"),
  ...
  },
  {
    ...
    clipBase: 0x1100,
    tileBase: 0x4000,
    upstreamSourcePath: path.join(petwarsUprightMaltipooDir, "maltipoo-max", "final", "spritesheet.png"),
    upstreamValidationPath: path.join(petwarsUprightMaltipooDir, "maltipoo-max", "final", "validation.json"),
...
const png = (canvas) => {
  ...
  for (let y = 0; y < canvas.height; y += 1) {
    ...
    raw[row] = 0;
    Buffer.from(canvas.data.subarray(y * canvas.width * 4, (y + 1) * canvas.width * 4)).copy(raw, row + 1);
  }
...
const assetLicense = {
  ...
  notes: "Maltipoo player sprites are reused from Petwars upright hatch-pet exports by Kevin Kern; BR object sprites are generated in-repo.",
...
const objectAssetLicense = {
  ...
  notes: "Programmatically generated Battle Royale pixel-art object content; no third-party source assets.",
  redistributable: true,
};
...
const weaponAssetLicense = {
  ...
  notes: "Petwars default weapon sprites generated for Kevin Kern and imported from app/public/assets/graphics/weapons.",
  redistributable: true,
};
...
const main = async () => {
  ...
  const manifest = {
    ...
    terrainTransitions: [],
    collisionMasks: [],
    placeables: [...buildPlayerPlaceables(), ...buildObjectPlaceables(), ...buildWeaponPlaceables(petwarsWeapons)],
  };

  await writeFile(path.join(outDir, "tileborne-asset-pack.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
  await writeFile(
