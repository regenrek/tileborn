...
describe("Battle Royale ability and status system", () => {
  it("dashes once per input sequence and arms the dash cooldown", () => {
    ...
    expect(world.getComponent<Position>(POSITION_COMPONENT).get(player)?.x).toBe(ABILITY.dash.distance);
    expect(world.getComponent<AbilityState>(ABILITY_STATE_COMPONENT).get(player)?.cooldowns).toEqual([
      { abilityId: ABILITY.dash.id, remainingTicks: ABILITY.dash.cooldownTicks },
    ]);
  ...
  it("shield burst absorbs canonical combat damage through the world view", () => {
    ...
    runSystem(world, new Map([["player-1", baseInput({ abilities: [ABILITY.shieldBurst.id] })]]), 1);
    ...
    const worldView = createBattleRoyaleCombatWorldView(world, {
      maxHealth: DAMAGE.playerHealth,
      footprintOffsetY: 0,
    }, []);
    worldView.setHealth(makeCombatEntityId(player), new HealthComponent({ current: 70, max: 100 }));

    expect(world.getComponent<Player>(PLAYER_COMPONENT).get(player)?.health).toBe(DAMAGE.playerHealth);
  ...
  it("scan pulse reveals hostile players in radius", () => {
    ...
    expect(world.getComponent<StatusEffects>(STATUS_EFFECTS_COMPONENT).get(target)?.effects).toContainEqual({
  ...
  it("trap deployables arm, trigger, and apply slow stun and damage-over-time", () => {
    ...
    runSystem(world, new Map([["player-1", baseInput({ abilities: [ABILITY.trap.id] })]]), 1, state);
    for (let tick = 2; tick <= ABILITY.trap.armTicks + 2; tick += 1) {
      runSystem(world, new Map(), tick, state);
    }

    const statuses = world.getComponent<StatusEffects>(STATUS_EFFECTS_COMPONENT).get(target)?.effects ?? [];
    ...
    expect([...world.getComponent<Deployable>(DEPLOYABLE_COMPONENT).entries()][0]?.[1].triggered).toBe(true);
  });
