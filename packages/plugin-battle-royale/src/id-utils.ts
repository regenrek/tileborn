import { makeLayerId, makeMapId, makeObjectId, sha256Hex, type MapId, type ObjectId, type Uuid } from "@tileborne/core";
...
export const uuidFromSeed = (seed: string | number, salt: string): Uuid => {
  ...
  const variant = ((Number.parseInt(digest.slice(16, 18), 16) & 0x3f) | 0x80).toString(16).padStart(2, "0");
  return [
    digest.slice(0, 8),
    digest.slice(8, 12),
...
export const layerIdFromSeed = (seed: string | number, salt: string) => makeLayerId(uuidFromSeed(seed, salt));
