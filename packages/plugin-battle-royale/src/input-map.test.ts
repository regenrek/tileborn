...
const actionState = (overrides: {
  ...
  digital: new Map((overrides.digital ?? []).map(([id, value]) => [coreActionId(id as never), value])),
  analog: new Map((overrides.analog ?? []).map(([id, value]) => [coreActionId(id as never), value])),
  pointer: new Map((overrides.pointer ?? []).map(([id, value]) => [coreActionId(id as never), value])),
});
