...
const clip = (
  ...
  frames: [
    { assetId: "playermodel:hero-atlas", uv: { x, y: 0, w: size.width, h: size.height }, durationMs: 100 },
    {
...
const modelRenderData = (
  ...
  clips: Object.fromEntries(
    REQUIRED_PLAYER_MODEL_CLIP_KEYS.map((key, index) => [key, clip(index * frameSize.width * 2, frameSize)]),
...
const expectedPlayerScale = (
  ...
  scaleX: (BATTLE_ROYALE_VISUAL_ORACLE.render.playerWorldFootprint.width * renderScale) / frameSize.width,
  scaleY: (BATTLE_ROYALE_VISUAL_ORACLE.render.playerWorldFootprint.height * renderScale) / frameSize.height,
});
...
describe("BattleRoyaleProjector", () => {
  ...
  it("projects three players and two projectiles into renderable entities", () => {
    ...
    expect(entities.filter((entity) => entity.id.startsWith("br:projectile-trail:"))).toHaveLength(2);
  ...
  it("derives short-lived impact effects when server delta removes projectiles", () => {
    ...
    expect(impactEntities.find((entity) => entity.id === "br:projectile:projectile-1")).toBeUndefined();
    ...
    expect(projector.project(expiredState).find((entity) => entity.id.startsWith("br:impact:"))).toBeUndefined();
  });
