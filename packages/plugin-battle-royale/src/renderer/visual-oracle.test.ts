...
describe("BATTLE_ROYALE_VISUAL_ORACLE", () => {
  it("captures the large-source-frame regression without accepting raw UV display size", () => {
    ...
    expect(exceedsSize(rawSourceFrame, BATTLE_ROYALE_VISUAL_ORACLE.render.maxPlayerScreenFootprint))
      .toBe(true);
    expect(exceedsSize(normalizedFootprint, BATTLE_ROYALE_VISUAL_ORACLE.render.maxPlayerScreenFootprint))
      .toBe(false);
  });
