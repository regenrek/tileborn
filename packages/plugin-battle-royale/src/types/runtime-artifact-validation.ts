...
const decodePlayerModels = (
  ...
  try {
    ...
    throw schemaError(cause instanceof Error ? cause.message : "invalid player model roster", location);
  }
};
  ...
  switch (role) {
    ...
    case "shrink-zone-anchor":
      return {
        ...
        role,
        properties: {
          initialRadiusTiles: readNumber(properties, "initialRadiusTiles", `${location}.properties`),
...
const decodeCollision = (
  input: unknown,
): BattleRoyaleArtifact["collision"] => {
  ...
  const chunks = readArray(record.chunks, "collision.chunks").map((chunk, index) => {
    ...
    return {
      ...
      tiles: readArray(chunkRecord.tiles, `collision.chunks[${index}].tiles`).map((tile, tileIndex) => {
...
const decodeRuntimeBattleRoyaleArtifact = (input: unknown): BattleRoyaleArtifact => {
  ...
  const artifact: BattleRoyaleArtifact = {
    ...
    objectPlacements: readArray(record.objectPlacements, "objectPlacements").map((placement, index) =>
      decodeObjectPlacement(placement, `objectPlacements[${index}]`),
    ),
    ...
    ...(record.playerModelSelections === undefined
      ? {}
      : {
          playerModelSelections: readArray(record.playerModelSelections, "playerModelSelections").map(
            (selection, index) => decodePlayerModelSelection(selection, `playerModelSelections[${index}]`),
          ),
        }),
  };
  return artifact;
...
const validateObjectPlacement = (
  ...
  if (!placement.objectId.startsWith("object:")) {
    issues.push(createValidationIssue("object placement id must be an ObjectId", `${location}.objectId`));
  }
  ...
  switch (placement.role) {
    case "spawn-point":
      if (placement.kind !== SPAWN_POINT_KIND) {
        issues.push(createValidationIssue("spawn placement kind must match the BR spawn object type", `${location}.kind`));
      }
      if (placement.properties.team.length === 0) {
        issues.push(createValidationIssue("spawn placement team is required", `${location}.properties.team`));
      }
      if (!isPositiveFinite(placement.properties.weight)) {
        issues.push(createValidationIssue("spawn placement weight must be positive", `${location}.properties.weight`));
      }
      break;
    case "shrink-zone-anchor":
      if (placement.kind !== SHRINK_ZONE_ANCHOR_KIND) {
        issues.push(
          createValidationIssue("shrink placement kind must match the BR shrink-anchor object type", `${location}.kind`),
        );
      }
    ...
    case "loot-crate":
      if (placement.kind !== LOOT_CRATE_KIND) {
        issues.push(createValidationIssue("loot placement kind must match the BR loot object type", `${location}.kind`));
      }
      if (placement.properties.itemKind.length === 0) {
        issues.push(createValidationIssue("loot placement item kind is required", `${location}.properties.itemKind`));
      }
      if (placement.properties.tier.length === 0) {
        issues.push(createValidationIssue("loot placement tier is required", `${location}.properties.tier`));
      }
      if (!isPositiveFinite(placement.properties.weight)) {
        issues.push(createValidationIssue("loot placement weight must be positive", `${location}.properties.weight`));
      }
      break;
    case "trap":
      if (placement.kind !== TRAP_KIND) {
        issues.push(createValidationIssue("trap placement kind must match the BR trap object type", `${location}.kind`));
      }
      if (!isPositiveFinite(placement.properties.radius)) {
        issues.push(createValidationIssue("trap radius must be positive", `${location}.properties.radius`));
      }
      if (!isNonNegativeFinite(placement.properties.slowTicks)) {
        issues.push(createValidationIssue("trap slow ticks must be non-negative", `${location}.properties.slowTicks`));
      }
      if (!isNonNegativeFinite(placement.properties.stunTicks)) {
        issues.push(createValidationIssue("trap stun ticks must be non-negative", `${location}.properties.stunTicks`));
      }
      if (!isNonNegativeFinite(placement.properties.damageTicks)) {
        issues.push(createValidationIssue("trap damage ticks must be non-negative", `${location}.properties.damageTicks`));
      }
      break;
    case "decoy":
      if (placement.kind !== DECOY_KIND) {
        issues.push(createValidationIssue("decoy placement kind must match the BR decoy object type", `${location}.kind`));
      }
      if (!isPositiveFinite(placement.properties.radius)) {
        issues.push(createValidationIssue("decoy radius must be positive", `${location}.properties.radius`));
      }
      if (!isPositiveFinite(placement.properties.durationTicks)) {
        issues.push(createValidationIssue("decoy duration must be positive", `${location}.properties.durationTicks`));
      }
      break;
  }
};
...
const validatePlayerModels = (
  ...
  models.forEach((model, index) => {
    if (modelIds.has(model.id)) {
      issues.push(createValidationIssue("player model ids must be unique", `playerModels[${index}].id`));
    }
  ...
  if (defaultModelId === undefined || defaultModelId.length === 0) {
    issues.push(createValidationIssue("default player model id is required", "defaultPlayerModelId"));
  } else if (!modelIds.has(defaultModelId)) {
    issues.push(createValidationIssue("default player model id must exist in playerModels", "defaultPlayerModelId"));
  }
  ...
  (artifact.playerModelSelections ?? []).forEach((selection, index) => {
    ...
    if (!modelIds.has(selection.modelId)) {
      issues.push(createValidationIssue("selected player model id must exist in playerModels", `${location}.modelId`));
    }
  });
};
...
const validateObjectCollisionRect = (
  ...
  if (!rect.objectId.startsWith("object:")) {
    issues.push(createValidationIssue("object collision objectId must be an ObjectId", `${location}.objectId`));
  }
  ...
  if (!isPositiveFinite(rect.width)) {
    issues.push(createValidationIssue("object collision width must be positive", `${location}.width`));
  }
  if (!isPositiveFinite(rect.height)) {
    issues.push(createValidationIssue("object collision height must be positive", `${location}.height`));
  }
  if (!rect.blocksMovement && !rect.blocksProjectiles && !rect.blocksVision) {
    issues.push(createValidationIssue("object collision rect must block at least one channel", location));
  }
};
...
export const validateDecodedBattleRoyaleArtifact = (
  ...
  if (artifact.spawnPoints.length !== artifact.spawnAnchors.length) {
    issues.push(createValidationIssue("spawnPoints and spawnAnchors must be index-aligned", "spawnPoints"));
  }
  artifact.spawnAnchors.forEach((point, index) => validateSpawnPoint(point, `spawnAnchors[${index}]`, issues));
  artifact.spawnPoints.forEach((point, index) => validateSpawnPoint(point, `spawnPoints[${index}]`, issues));
  ...
  if (!isPositiveFinite(shrinkSchedule.startRadiusTiles)) {
    issues.push(createValidationIssue("shrink start radius must be positive", "shrinkSchedule.startRadiusTiles"));
  }
  if (!isNonNegativeFinite(shrinkSchedule.endRadiusTiles)) {
    issues.push(createValidationIssue("shrink end radius must be non-negative", "shrinkSchedule.endRadiusTiles"));
  }
  if (shrinkSchedule.endRadiusTiles >= shrinkSchedule.startRadiusTiles) {
    issues.push(createValidationIssue("shrink end radius must be below start radius", "shrinkSchedule.endRadiusTiles"));
  }
  if (!isPositiveFinite(shrinkSchedule.shrinkIntervalMs)) {
    issues.push(createValidationIssue("shrink interval must be positive", "shrinkSchedule.shrinkIntervalMs"));
  }
  if (!isNonNegativeFinite(shrinkSchedule.damagePerSecond)) {
    issues.push(createValidationIssue("shrink damage must be non-negative", "shrinkSchedule.damagePerSecond"));
  }
  ...
  artifact.objectPlacements.forEach((placement, index) => validateObjectPlacement(placement, index, issues));
  ...
  if (artifact.collision) {
    ...
    if (!isPositiveFinite(artifact.collision.tileWidth)) {
      issues.push(createValidationIssue("collision tileWidth must be positive", "collision.tileWidth"));
    }
    if (!isPositiveFinite(artifact.collision.tileHeight)) {
      issues.push(createValidationIssue("collision tileHeight must be positive", "collision.tileHeight"));
    }
    if (artifact.collision.chunks.length === 0) {
      issues.push(createValidationIssue("collision chunks are required when collision is present", "collision.chunks"));
    }
    if (artifact.collision.tileIdByIndex.length === 0) {
      issues.push(createValidationIssue("collision tileIdByIndex must not be empty", "collision.tileIdByIndex"));
    }
    artifact.collision.chunks.forEach((chunk, index) => {
      const location = `collision.chunks[${index}]`;
      if (!isPositiveFinite(chunk.width)) {
        issues.push(createValidationIssue("collision chunk width must be positive", `${location}.width`));
      }
      if (!isPositiveFinite(chunk.height)) {
        issues.push(createValidationIssue("collision chunk height must be positive", `${location}.height`));
      }
      if (chunk.tiles.length !== chunk.width * chunk.height) {
        issues.push(createValidationIssue("collision chunk tiles must match width * height", `${location}.tiles`));
      }
      chunk.tiles.forEach((tile, tileIndex) => {
        if (!Number.isInteger(tile) || tile < 0) {
          issues.push(createValidationIssue("collision tile index must be a non-negative integer", `${location}.tiles[${tileIndex}]`));
        }
      });
    });
  }
