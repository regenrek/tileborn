...
const brManifestPath = path.join(packageRoot, "..", "plugin-battle-royale", "tileborne-plugin.json");
...
describe("example-arena proves genre-neutral extensibility", () => {
  ...
  describe("each contributed contract decodes against the engine schema", () => {
    it("inputMap decodes via decodeInputMap and differs from BR's default map", () => {
      ...
      const bindings =
        decoded.success.schemeDefaults["keyboard-mouse" as keyof typeof decoded.success.schemeDefaults] ??
        [];
      ...
      expect(primary.some((b) => b.trigger._tag === "mouseButton" && b.trigger.button === 0)).toBe(true);
      expect(primary.some((b) => b.trigger._tag === "key" && b.trigger.code === "Space")).toBe(false);
