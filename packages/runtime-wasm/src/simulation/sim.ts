...
export const runSimulationTick = (
  ...
  Effect.gen(function* () {
    if (!Number.isInteger(request.tick) || request.tick < 0) {
      return yield* Effect.fail(new SimulationInputError({ message: "tick must be a non-negative integer" }));
    }
    if (!Number.isFinite(request.deltaSeconds) || request.deltaSeconds < 0) {
      return yield* Effect.fail(new SimulationInputError({ message: "deltaSeconds must be a finite non-negative number" }));
    }
    return [
      {
        kind: "tick",
