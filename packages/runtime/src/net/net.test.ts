...
describe('NetClient', () => {
  ...
  it('reconnects after an unexpected close and resumes the receive stream', async () => {
    ...
    const messages = await Effect.runPromise(
      client.receive().pipe(Stream.take(1), Stream.runCollect),
    );
  ...
  it('caps reconnect attempts across immediate re-close events', async () => {
    ...
    const transport = makeBrowserWebSocketTransport({
      ...
      fetch: async () => {
        reconnectFetches += 1;
        return jsonResponse({
          wsUrl: `wss://reconnect-${reconnectFetches}`,
          reconnectToken: 'token-1',
        });
      },
    });
  ...
  it('surfaces decode failures as typed ProtocolError failures', async () => {
    const client = makeNetClient(
      mockTransport({ events: [{ _tag: 'message', data: new Uint8Array([0xff]) }] }),
    );
...
const makeFakeWebSocket = (sockets: FakeWebSocket[], onOpened: (socket: FakeWebSocket) => void) =>
