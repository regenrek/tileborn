...
describe('PluginHost', () => {
  ...
  it('dispatches onTick to all plugins in insertion order', async () => {
    ...
    await Effect.runPromise(
      host.register(plugin('a', { onTick: () => Effect.sync(() => calls.push('a')) })),
    );
    await Effect.runPromise(
      host.register(plugin('b', { onTick: () => Effect.sync(() => calls.push('b')) })),
    );
  ...
  it('continues dispatching when one plugin hook fails', async () => {
    ...
    await Effect.runPromise(
      host.register(plugin('good', { onTick: () => Effect.sync(() => calls.push('good')) })),
    );
    await Effect.runPromise(host.dispatchTick(new World(), 1 / 60, 1));
  ...
  it('dispatches onMessage for inbound server messages', async () => {
    ...
    await Effect.runPromise(
      host.register(
        plugin('listener', { onMessage: (message) => Effect.sync(() => calls.push(message._tag)) }),
      ),
    );
