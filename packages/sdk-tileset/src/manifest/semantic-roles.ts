...
const inferredRolesForTile = (tile: Tile): readonly AssetSemanticRoleName[] => {
  ...
  const terrainClass =
    tile.terrainClass._tag === 'Some' ? tile.terrainClass.value.toLowerCase() : undefined;
  ...
  if (terrainClass !== undefined) {
    ...
    if (
      role !== undefined &&
      (role !== 'floor' || (!roles.has('water') && !tags.has('transparent')))
    ) {
      roles.add(role);
    }
  }
