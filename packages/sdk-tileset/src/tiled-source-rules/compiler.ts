...
const stableRuleTileOrder = (
  left: TiledSourceTiledLayerTile,
  right: TiledSourceTiledLayerTile,
): number =>
  left.row - right.row ||
...
const stableCompiledRuleOrder = (
  left: TiledSourceCompiledAutomappingRule,
  right: TiledSourceCompiledAutomappingRule,
): number =>
  left.phaseOrder - right.phaseOrder ||
  left.path.localeCompare(right.path) ||
  left.wall.localeCompare(right.wall, undefined, { numeric: true });

const stableCellOrder = (
...
): readonly TiledSourceCompiledRuleTile[] =>
  [...tiles].sort(stableRuleTileOrder).map(
    (tile) =>
...
const layerNamed = (
  ...
  name: LayerId,
): TiledSourceTiledTileLayer | undefined => layers.find((layer) => layer.name === name);

const objectGroupNamed = (
  groups: readonly TiledSourceTiledObjectGroup[],
  name: LayerId,
): TiledSourceTiledObjectGroup | undefined => groups.find((group) => group.name === name);
...
const layerDiagnostics = (
  ...
  if (inputLayer === undefined) {
    diagnostics.push(
      new MissingRuleLayerDiagnostic({
        severity: 'error',
  ...
  if (rule.phase === 'unknown') {
    diagnostics.push(
      new UnsupportedRulePhaseDiagnostic({
        severity: 'warning',
...
const compileRuleOptions = Effect.fn('TiledSourceRules.compileRuleOptions')(function* (
  ...
  for (const object of group.objects) {
    ...
    options.push(
      new TiledSourceCompiledRuleOption({
        id: object.id,
        x: object.x,
        y: object.y,
        width: object.width,
...
const assertNoContradictoryRulePaths = Effect.fn('TiledSourceRules.assertNoContradictoryRulePaths')(
  function* (rules: readonly TiledSourceAutomappingRule[]) {
...
const compileAutomappingRule = Effect.fn('TiledSourceRules.compileAutomappingRule')(function* (
  ...
  const options = yield* compileRuleOptions(
    rule,
    objectGroupNamed(rule.objectGroupDetails, RULE_OPTIONS),
  );
  const diagnostics = [
...
const compileWangSets = (
  manifest: TiledSourceTiledSourceManifest,
): readonly TiledSourceCompiledWangSet[] =>
  manifest.tilesets.flatMap((tileset) =>
    tileset.wangSets.map(
      (wangSet) =>
...
const compileWallGroups = (
  rules: readonly TiledSourceCompiledAutomappingRule[],
): readonly TiledSourceCompiledWallGroup[] => {
...
export const compileTiledSourceRulePipeline: (
  ...
  const digestInput = {
    ...
    automappingRules: automappingRules.map((rule) =>
      encodeForHash(TiledSourceCompiledAutomappingRule, rule),
    ),
    wallGroups: wallGroups.map((wallGroup) =>
      encodeForHash(TiledSourceCompiledWallGroup, wallGroup),
    ),
    diagnostics: diagnostics.map((diagnostic) =>
      encodeForHash(TiledSourceRuleDiagnostic, diagnostic),
    ),
  };
...
const ruleMatchesCell = (
  ...
  return (
    sourceId !== undefined && rule.inputTiles.some((tile) => ruleTileAssetKey(tile) === sourceId)
  );
};
...
export const collectProjectedAssetKeys = (
