...
import { exportTiledMapToTmj, metadataToTiledProperties, type TmjExportTileset } from "../export-tmj.js";
...
const parse = (raw: string, sourcePath: string) => {
  ...
  if (!result.value) throw new Error(`fixture failed to import: ${JSON.stringify(result.diagnostics)}`);
  return result.value;
};
...
describe("metadataToTiledProperties", () => {
  it("infers Tiled property types from primitive carriers and sorts keys", () => {
    expect(
      metadataToTiledProperties({ flag: true, ratio: 0.5, count: 3, label: "hi" }),
    ).toEqual([
...
describe("tiled TMJ export round-trip", () => {
  ...
  it("preserves multiple tilesets with gapped firstgids", () => {
    const raw = JSON.stringify({
      ...
      tileheight: 16,
      tilesets: [
        { firstgid: 1, name: "a", tilewidth: 16, tileheight: 16, tilecount: 1, columns: 1, image: "a.png", imagewidth: 16, imageheight: 16 },
        { firstgid: 100, name: "b", tilewidth: 16, tileheight: 16, tilecount: 1, columns: 1, image: "b.png", imagewidth: 16, imageheight: 16 },
      ],
