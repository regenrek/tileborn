  ...
  tiles: [
    {
      id: 1,
      probability: 0.25,
      animation: [{ tileid: 2, duration: 100 }, { tileid: 3, duration: 150 }],
...
describe("tiled import", () => {
  ...
  it("imports standalone Atlas-Props-style TSX grid tilesets as tileset packs", async () => {
    ...
    expect(value?.pack.assets.map((asset) => asset.path)).toContain("TiledMap Editor/Images/Atlas-Props.png");
  ...
  it("imports standalone TSJ tilesets through the same SDK path", async () => {
    ...
    expect(value?.pack.assets.map((asset) => asset.path)).toContain("TiledMap Editor/Images/Atlas-Props-Json.png");
  });
  ...
  it("keeps Atlas-Props-Sprites-style sparse image-collection tile IDs", async () => {
    ...
    const value = result.value as TiledTilesetPackImport | undefined;
    expect(value?.kind).toBe("tileset-pack");
    expect(value?.scan.placeableCandidates.map((candidate) => candidate.localTileId)).toEqual([1618, 2258]);
    expect(value?.pack.placeables?.map((placeable) => placeable.source.localTileId)).toEqual([1618, 2258]);
  ...
  it("blocks standalone tileset image paths that escape the project root", async () => {
    const unsafe = atlasPropsSpritesTsx.replace("../Images/Props/crate.png", "../../../outside/crate.png");
    const result = await importTiled(
      {
        sourcePath: `${PROJECT_ROOT}/TiledMap Editor/Tilesets/Atlas-Props-Sprites.tsx` as const,
        projectRoot: PROJECT_ROOT,
  ...
  it("imports raw Tiled source folders as source packs and preserves rules", async () => {
    ...
    expect(value?.pack.placeables?.map((placeable) => placeable.source.localTileId)).toEqual([1618, 2258]);
    expect(value?.rules.map((rule) => ({ kind: rule.kind, path: rule.path.replace(PROJECT_ROOT, "") }))).toEqual(
      expect.arrayContaining([
  ...
  it("honors tileborne.anchor center and top-left under standard-plus-hints", async () => {
    ...
    expect(
      result.value?.map.objects.every((object) => object.properties["tileborne.anchor"] === "top-left"),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    ["/etc/passwd"],
    ["./a/../../b.png"],
  ])("blocks escaping image-collection image source %s", async (imageSource) => {
    const result = await importTiled(
      {
  ...
  it("promotes hinted atlas tiles only under standard-plus-hints", async () => {
    const standard = await importTiled(
      { sourcePath: `${PROJECT_ROOT}/maps/hinted.tmj`, projectRoot: PROJECT_ROOT, raw: hintedAtlasTmj },
      { packIdSeed: PACK_SEED, profile: "standard" },
    );
  ...
  it("diagnoses unmarked atlas object regions without auto-promoting them", async () => {
    ...
    expect(result.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ _tag: "TiledAmbiguousAtlasObject" }),
      ]),
    );
  });
  ...
  it.each([
    ...
    [
      ...
      tiledMapWith({
        layers: [
          { type: "objectgroup", name: "objects", objects: [{ id: 1, x: 0, y: 0, template: "tree.tx" }] },
        ],
      }),
      "templates",
    ],
    ...
    [
      "parallax",
      tiledMapWith({
        layers: [{ type: "tilelayer", name: "ground", width: 1, height: 1, data: [1], parallaxx: 0.5 }],
      }),
      "parallax",
    ],
    [
  ...
  it("builds deterministic import plans and isolates assistive inference until accepted", async () => {
    ...
    expect(JSON.stringify(buildImportPlan(scanned.scan!, "standard"))).toBe(JSON.stringify(standard));
    expect(standard.suggestions).toEqual([]);
    expect(assistive.suggestions[0]).toEqual(expect.objectContaining({
  ...
  it("adds SDK-owned source role recommendations to scans and plans", async () => {
    ...
    expect(objectLayer.scan?.sourceRoles).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ kind: "map-context", evidence: "object-layer", reviewRequired: false }),
      ]),
    );
    ...
    expect(ambiguous.scan?.sourceRoles).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ kind: "paintable-tileset", evidence: "grid-tileset" }),
        expect.objectContaining({ kind: "review-required", evidence: "ambiguous-atlas-object", reviewRequired: true }),
      ]),
    );
