...
export class ProjectBehaviorService extends Context.Service<ProjectBehaviorService, {
  readonly open: (
    projectId: ProjectId,
...
const decodeRegistryDocument = (value: unknown): ProjectBehaviorRegistryDocument =>
  Schema.decodeUnknownSync(
    ProjectBehaviorRegistryDocument as never,
  )(value) as ProjectBehaviorRegistryDocument;
...
const nestedReferences = (
...
): readonly { readonly reference: BehaviorReference; readonly path: string; readonly nodeId?: BehaviorNodeId }[] => {
  if (typeof value !== 'object' || value === null) return [];
  if (Array.isArray(value)) {
    return value.flatMap((entry, index) => nestedReferences(entry, `${pathPrefix}.${index}`, nodeId));
  }
  ...
  const found =
    record._tag === 'behavior' && typeof record.behaviorId === 'string'
      ? [{ reference: record as unknown as BehaviorReference, path: pathPrefix, ...(currentNodeId === undefined ? {} : { nodeId: currentNodeId }) }]
      : [];
  return [
    ...found,
    ...Object.entries(record).flatMap(([key, entry]) =>
      nestedReferences(entry, pathPrefix.length === 0 ? key : `${pathPrefix}.${key}`, currentNodeId),
    ),
  ];
};

const useSitesFrom = (resources: readonly ProjectBehaviorResource[]): readonly ProjectBehaviorUseSite[] =>
...
export const makeProjectBehaviorServiceLive = (
  ...
  Effect.gen(function* () {
    ...
    const withProjectWrite = <A, E, R>(
      ...
      (gate) => Effect.uninterruptibleMask((restore) =>
      ...
      Effect.tryPromise({
        try: async () => {
          if (manifest.source._tag === 'visual') {
            ...
            const raw = await readFile(path.join(projectRoot, manifest.source.definitionPath), 'utf8');
            const definition = decodePersistedBehaviorDefinitionJson(JSON.parse(raw));
            if (definition.id !== manifest.id) throw new Error(`visual behavior id mismatch: ${manifest.id}`);
            return {
              kind: 'visual',
          ...
          const source = await readFile(path.join(projectRoot, manifest.source.sourcePath), 'utf8');
          return {
            kind: 'typescript',
            manifest: manifest as BehaviorManifest & { readonly source: TypeScriptBehaviorSource },
            source,
          };
        },
        catch: (cause) =>
          ...
          if (registry.trust === 'imported-untrusted' && manifest.source._tag === 'typescript') {
            diagnostics.push(
              behaviorDiagnostic({
                ...
                message: 'Imported TypeScript behavior code is disabled until the project is trusted.',
                behaviorId: manifest.id,
    ...
    const writeRegistryPromise = (
      projectRoot: string,
      registry: ProjectBehaviorRegistryDocument,
    ): Promise<void> => persistence.writeTextAtomic(
    ...
    const assertTransactionBinding = (
      ...
      if (baseRegistry.projectId !== journal.projectId || nextRegistry.projectId !== journal.projectId) {
      ...
      if (
        new Set(baseRegistry.entries.map((entry) => entry.id)).size !== baseRegistry.entries.length ||
        new Set(nextRegistry.entries.map((entry) => entry.id)).size !== nextRegistry.entries.length
      ) {
      ...
      if ((baseEntry !== undefined) !== expectedBasePresence || (nextEntry !== undefined) !== expectedNextPresence) {
      ...
      const assertManifestSource = (
        ...
        const manifestPath = manifest.source._tag === 'visual'
          ? manifest.source.definitionPath
          : manifest.source.sourcePath;
      ...
      if (journal.operation === 'convert') {
        if (
          ...
          journal.previousSourcePath !== canonicalSourcePath(journal.behaviorId, journal.previousSourceKind)
        ) {
        ...
        if (
          ...
          JSON.stringify(baseEntry.requiredCapabilities) !== JSON.stringify(nextEntry.requiredCapabilities)
        ) {
          throw new Error('behavior conversion transaction may only change canonical source');
        }
      } else {
        if (baseEntry !== undefined) assertManifestSource(baseEntry, journal.sourceKind, expectedSourcePath);
        if (nextEntry !== undefined) assertManifestSource(nextEntry, journal.sourceKind, expectedSourcePath);
      }
      ...
      if (journal.operation === 'remove') {
        ...
        if (journal.sourceKind === 'visual') {
          const definition = decodePersistedBehaviorDefinitionJson(JSON.parse(journal.nextSource) as unknown);
      ...
      if (
        ((journal.operation === 'create' || journal.operation === 'convert') && journal.baseSourceExists) ||
        (journal.operation !== 'create' && journal.operation !== 'convert' && !journal.baseSourceExists)
      ) {
    ...
    const decodeTransaction = (
      ...
      if (
        ...
        !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(journal.transactionId) ||
    ...
    const recoverTransactionPromise = async (
      ...
      const currentRegistry = currentRaw === undefined
      ...
      if (currentRegistry.projectId !== projectId) {
        ...
        throw new Error(`conflicting behavior transaction quarantined at ${quarantined}: current project mismatch`);
      }
      ...
      if (sourceFile !== expectedSourceFile) {
        const quarantined = await quarantineTransaction(journalFile, journal.transactionId);
        throw new Error(`conflicting behavior transaction quarantined at ${quarantined}: source path mismatch`);
      }
      const previousSourceFile = journal.operation === 'convert'
        ? path.resolve(projectRoot, journal.previousSourcePath!)
        : undefined;
      if (
        journal.operation === 'convert' &&
        previousSourceFile !== path.resolve(
          projectRoot,
          canonicalSourcePath(journal.behaviorId, journal.previousSourceKind!),
        )
      ) {
        const quarantined = await quarantineTransaction(journalFile, journal.transactionId);
        throw new Error(`conflicting behavior transaction quarantined at ${quarantined}: previous source path mismatch`);
      }
      ...
      if (currentHash === journal.nextRegistryHash) {
        ...
        if (currentSourceHash !== journal.nextSourceHash) {
          const quarantined = await quarantineTransaction(journalFile, journal.transactionId);
          throw new Error(`conflicting behavior transaction quarantined at ${quarantined}: committed source mismatch`);
        }
        if (previousSourceFile !== undefined) {
          ...
          if (previousSource !== undefined) {
            if (sourceContentHash(previousSource) !== journal.previousSourceHash) {
              const quarantined = await quarantineTransaction(journalFile, journal.transactionId);
              throw new Error(`conflicting behavior transaction quarantined at ${quarantined}: previous source mismatch`);
            }
      ...
      if (currentHash !== journal.baseRegistryHash) {
        const reason = currentRegistry.revision > journal.nextRegistry.revision ? 'stale' : 'conflicting';
        const quarantined = await quarantineTransaction(journalFile, journal.transactionId);
      ...
      if (!sourceMatchesBase && !sourceMatchesNext) {
        const quarantined = await quarantineTransaction(journalFile, journal.transactionId);
        throw new Error(`conflicting behavior transaction quarantined at ${quarantined}: base source mismatch`);
      }
    ...
    const recoverTransaction = (projectRoot: string, projectId: ProjectId) =>
      Effect.tryPromise({
        ...
        catch: (cause) => new ProjectBehaviorError({
          projectId,
    ...
    const commitTransaction = (
      ...
      try: async () => {
        ...
        const previousSourceFile = previousSource === undefined
          ? undefined
          : path.join(projectRoot, previousSource.sourcePath);
        const previousSourceContents = previousSourceFile === undefined
          ? undefined
        ...
        try {
          ...
          try {
            ...
            const recoveryMessage = recovery instanceof Error ? recovery.message : String(recovery);
      ...
      catch: (cause) => cause instanceof ProjectBehaviorTransactionError
        ? cause
        : new ProjectBehaviorError({
            projectId,
            path: TRANSACTION_FILE,
    ...
    const createVisual = (projectId: ProjectId, input: CreateVisualProjectBehaviorInput) =>
      withProjectWrite(projectId, Effect.gen(function* () {
        const projectRoot = yield* rootFor(projectId);
        yield* recoverTransaction(projectRoot, projectId);
        const registry = yield* readRegistry(projectRoot, projectId);
    ...
    const createTypeScript = (projectId: ProjectId, input: CreateTypeScriptProjectBehaviorInput) =>
      withProjectWrite(projectId, Effect.gen(function* () {
        const projectRoot = yield* rootFor(projectId);
        yield* recoverTransaction(projectRoot, projectId);
        const registry = yield* readRegistry(projectRoot, projectId);
    ...
    const saveVisual = (input: SaveVisualProjectBehaviorInput) =>
      withProjectWrite(input.projectId, Effect.gen(function* () {
        ...
        yield* commitTransaction(
          ...
          registry,
          new ProjectBehaviorRegistryDocument({ ...registry, revision: registry.revision + 1, entries }),
        );
    ...
    const remove = (
      ...
      withProjectWrite(projectId, Effect.gen(function* () {
        ...
        const sourcePath = entry.source._tag === 'visual' ? entry.source.definitionPath : entry.source.sourcePath;
        yield* commitTransaction(
          projectRoot,
          projectId,
          'remove',
