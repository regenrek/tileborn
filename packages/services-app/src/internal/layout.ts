...
export const projectManifestPath = (projectDir: string): string =>
  path.join(projectDir, 'project.json');

export const projectLockPath = (projectDir: string): string =>
  path.join(projectDir, 'project.lock.json');
...
export const packDirectoryName = (packId: PackId, version: string): string =>
  `${packId}-${version}`;
