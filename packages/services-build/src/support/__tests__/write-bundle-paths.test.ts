...
const supportLayer = SupportServiceLive.pipe(
  Layer.provideMerge(Layer.mergeAll(HomeServiceLive, JobServiceLive)),
);
...
describe('SupportService writeBundle path security', () => {
  ...
  it('writeBundle rejects symlink destinations outside the tileborne home root', () =>
    withTempHome(async (home) => {
      ...
      const error = await runSupport(writeBundle('escape-link/support.tar.gz')).catch(
        (cause) => cause,
      );
