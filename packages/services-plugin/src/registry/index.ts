...
export class PluginRegistryService extends Context.Service<
  PluginRegistryService,
  {
    ...
    readonly getManifest: (
      pluginId: PluginId,
    ) => Effect.Effect<PluginManifest, PluginRegistryServiceError>;
    readonly verify: (
      pluginId?: PluginId,
    ) => Effect.Effect<readonly PluginVerifyResult[], PluginRegistryServiceError>;
...
const readPluginDirectoryEntries = (
  ...
  Effect.tryPromise({
    ...
    catch: (cause) => new PluginValidationError({ path: pluginsPath, message: toMessage(cause) }),
  });
...
export const PluginRegistryServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const readVerifiedPlugins = Effect.fn('PluginRegistryService.readVerifiedPlugins')(
      function* () {
        yield* home.init();
        ...
        return [...new Map(discovered.map((plugin) => [plugin.id, plugin])).values()].sort(
          (left, right) => left.id.localeCompare(right.id),
        );
      },
    );

    const readVerifiedPluginById = Effect.fn('PluginRegistryService.readVerifiedPluginById')(
      function* (pluginId: PluginId) {
        ...
        return yield* readInstalledPlugin(
          rootPath,
    ...
    const getManifest = Effect.fn('PluginRegistryService.getManifest')(function* (
      pluginId: PluginId,
    ) {
