...
const pluginId = (id = '@tileborne-plugins/test'): PluginId =>
  Schema.decodeUnknownSync(PluginId)(id);
...
const writePluginSource = async (
  ...
  await writeFile(
    ...
    `${JSON.stringify(manifest(id, version, extra), null, 2)}\n`,
  );
...
const runWithInstaller = <A, E>(
  effect: Effect.Effect<A, E, PluginInstallerService | PluginRegistryService>,
...
describe.sequential('PluginRegistryService', () => {
  it('discovers an empty home', async () => {
    await makeTempHome();
    const plugins = await runWithRegistry(
      Effect.gen(function* () {
  ...
  it('returns a typed manifest by plugin id', async () => {
    const home = await makeTempHome();
    await writeInstalledPlugin(home);
    const found = await runWithRegistry(
      Effect.gen(function* () {
        const registry = yield* PluginRegistryService;
        yield* registry.discover();
  ...
  it('disables plugins and persists preferences', async () => {
    const home = await makeTempHome();
    await writeInstalledPlugin(home);
    const disabled = await runWithRegistry(
      Effect.gen(function* () {
        const registry = yield* PluginRegistryService;
        yield* registry.discover();
  ...
  it('enables plugins and persists preferences', async () => {
    const home = await makeTempHome();
    await writeInstalledPlugin(home);
    const enabled = await runWithRegistry(
      Effect.gen(function* () {
        const registry = yield* PluginRegistryService;
        yield* registry.discover();
  ...
  it('emits subscription snapshots on changes', async () => {
    const home = await makeTempHome();
    await writeInstalledPlugin(home);
    const snapshots = await runWithRegistry(
      Effect.gen(function* () {
        const registry = yield* PluginRegistryService;
...
describe('validateRelativePluginPath', () => {
  it('rejects raw traversal segments before normalization', () => {
    expect(() => validateRelativePluginPath('/root', 'dir/../entry.js')).toThrow(
      PluginValidationError,
    );
  ...
  it('accepts current-shape relative plugin paths', () => {
    ...
    expect(validateRelativePluginPath('/root', 'a/b/c.js')).toBe(
      path.join('/root', 'a', 'b', 'c.js'),
    );
  });
});
...
describe.sequential('PluginInstallerService', () => {
  it('installs a local source through staging and atomic rename', async () => {
    ...
    try {
      const installed = await runWithInstaller(
        Effect.gen(function* () {
      ...
      expect(
        installed.rootPath.endsWith(
          expectedPluginDirectoryName('@tileborne-plugins/test', '0.1.0'),
        ),
      ).toBe(true);
    } finally {
  ...
  it('rejects manifests missing required fields', async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ id: '@tileborne-plugins/test' }),
    );
    try {
      await expect(
        runWithInstaller(
  ...
  it('replaces a non-empty existing plugin directory during install', async () => {
    ...
    try {
      ...
      await expect(
        readFile(path.join(existing, 'tileborne-plugin.json'), 'utf8'),
      ).resolves.toContain('@tileborne-plugins/test');
  ...
  it('serializes concurrent installs of the same plugin', async () => {
    await makeTempHome();
    const sources = await Promise.all(
      Array.from({ length: 4 }, async (_, sourceIndex) => {
        const source = await mkdtemp(path.join(tmpdir(), 'tileborne-plugin-source-'));
        await writePluginSource(source);
        await Promise.all(
          Array.from({ length: 40 }, (_, fileIndex) =>
            writeFile(
              path.join(source, `content-${fileIndex}.txt`),
              `source ${sourceIndex} file ${fileIndex}\n`,
            ),
          ),
        );
        return source;
    ...
    try {
      const plugins = await runWithInstaller(
        Effect.gen(function* () {
          const installer = yield* PluginInstallerService;
          const registry = yield* PluginRegistryService;
          yield* Effect.all(
            sources.map((source) => installer.install(new LocalPluginSource({ path: source }))),
    ...
    await expect(
      Effect.runPromise(
        Effect.gen(function* () {
          ...
          yield* registry.discover();
          return yield* loader.loadExecutable(pluginId());
        }).pipe(Effect.provide(PluginLoaderRendererLayer)),
      ),
    ).rejects.toMatchObject({
