...
describe("clean-checkout-time report validation", () => {
  ...
  it("rejects a fixture that exceeds an install timing limit", () => {
    ...
    expect(() => assertCleanCheckoutTimeBoundaries(report)).toThrow(/install --frozen-lockfile exceeded limit/);
  });
  ...
  it("rejects a fixture that exceeds total wall time", () => {
    ...
    expect(() => assertCleanCheckoutTimeBoundaries(report)).toThrow(/total duration exceeded limit/);
  });
